...
> 本项目（cortex-agent）基于 zavora-ai 旗舰项目 [`adk-rust`](https://github.com/zavora-ai/adk-rust) 构建，且已内置完整 MCP 管理功能（前后端 + 种子机制，见 [mcp-management.md](./design/mcp-management.md)）。
>
...
- `call_tool` 60s 超时（MCP 工具卡死时返回错误，不无限阻塞 SSE，参考 codex `DEFAULT_TOOL_TIMEOUT`）
...
2. **注册到 cortex**（二选一）：
   - **配置驱动**：`config/config.local.toml` 加 `[[mcp.seeds]]`，启动自动 upsert：
     ```toml
     [[mcp.seeds]]
     slug = "memory"
     name = "会话记忆"
