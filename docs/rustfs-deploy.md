...
> S3 兼容对象存储服务器，作为 cortex-agent 的**共享文件层**：承载截图（`screenshots/`）、上传图（`uploads/`）、沙箱快照（`workspaces/`）。（artifact 暂走本地文件系统 `{data_dir}/artifacts`，对象存储接入预留三期。）Apache 2.0、Rust 实现，可作为 MinIO 的二进制替换。官方仓库：[rustfs/rustfs](https://github.com/rustfs/rustfs)，官方文档：<https://docs.rustfs.com>。
>
...
> 此处 `RUSTFS_ACCESS_KEY` / `RUSTFS_SECRET_KEY` 是 RustFS 的**根凭证**，须与 cortex-agent 的 `config/config.toml` → `[object_storage]` 的 `access_key` / `secret_key` **保持一致**。下面用项目示例值 `cortex` / `cortex12345`（取自开发默认配置 `config/config_1.toml`；生产改 `config/config.toml` 时，`/etc/default/rustfs`、`config.toml`、控制台/mc 三处需一致）。
...
> ⚠️ **桶必须先建，否则对象存储功能整体降级**。
> cortex-agent 的对象存储客户端启动时会跑连通性自检（`src/infra/object_store.rs` 的 `op.check()`），若 `config.toml` 配置的桶（`cortex`）不存在，自检失败——启动日志出现 error、`object_store` 降级为 `None`（截图 503、上传报「对象存储未启用，无法上传」、沙箱快照不可用）。**主程序仍会启动**（降级非致命，见 DEPLOY.md §七降级表），但运行期不会自愈，需建桶后重启 cortex-agent。排查见 §九 Q2。
>
> **为什么不能代码自动建桶**：项目用的 opendal `0.58` 只能操作「已存在的桶」——其 `S3Builder` 没有 `auto_create_bucket`、`Operator` 也不暴露 `create_bucket`（0.51.2 时代即如此，0.58 仍未提供）。所以 `cortex` 桶必须在部署阶段手动（或脚本）建一次。详见 §九 Q4。
...
### Q2 cortex-agent 启动日志报「对象存储连通性自检失败」

`src/infra/object_store.rs` 启动会执行 `op.check()`，失败只进日志并降级（主程序仍可启动，见 §四）。常见三类原因：
...
只想跳过对象存储做最小本地调试：`config.toml` 设 `[object_storage].enabled = false`（截图/上传图/沙箱快照不可用，主程序仍可启动）。自检失败与 enabled=false 的区别：前者会在启动日志留下 error（运行期不自愈，修好需重启），后者干净跳过。
...
当前**不能**。opendal `0.58` 的 `S3Builder` 没有 `auto_create_bucket`、`Operator` 也不暴露 `create_bucket`——它只能操作已存在的桶（0.51.2 时代即如此，0.58 仍未提供）。所以 `cortex` 桶必须在部署阶段手动/脚本建一次（见 §四）。

> 将来若要做「代码自动建桶」：依赖树里已有 opendal 传递进来的 AWS SigV4 签名实现（升级后重组为 `reqsign-aws-v4` 3.x / `reqsign-core` 3.x，旧 `reqsign 0.16` 单 crate 已不存在）与 `hmac`/`sha2`，可在 `object_store.rs` 自检前用 `reqwest` + 签名发一个 `PUT /cortex` 建桶请求（需在 `Cargo.toml` 显式声明对应 crate——间接依赖在 Rust 2024 edition 不自动暴露）。现阶段手动建桶已足够。
...
### Q6 运行中上传报「对象存储未启用，无法上传」但 RustFS 明明在跑

> 高频踩坑，**报错文案有误导性**。与 Q2 同源（都是自检失败导致 `object_store=None`），但 Q2 是「启动时报」，本条是「cortex 已正常跑起来、上传时才报」。

报错来自 `src/server/mod.rs` 的 `handle_upload`：当 cortex 进程内的 `object_store` 字段为 `None` 时直接返回此文案。**它不等于 RustFS 服务没起**——`None` 只有两个来源：

| 来源 | 启动日志表现 |
|---|---|
| `[object_storage].enabled = false` | `[infra] 对象存储未启用(enabled=false)` |
| **启动时初始化失败**（自检没过，即使 RustFS 此刻在跑） | `[infra] 对象存储初始化失败(...)`，且**没有** `[infra] object store 就绪:...` 这行 |

> ⚠️ **关键机制：cortex 只在启动那一刻做连通性自检一次，运行期永不重连**（自检逻辑见 Q2）。所以两种典型时序都会让它「卡在 None」，哪怕你后来把 RustFS / 桶都补齐了：
> - **cortex 先起、RustFS 后起** → cortex 启动自检时连不上 → `None`；之后 RustFS 起来了 cortex 也不会重连。
> - **cortex 启动时 `cortex` 桶还没建** → 自检失败 → `None`；后来桶建了也不会恢复。
>
> 这正是「RustFS 明明在跑却报未启用」的根因——旧进程的 `None` 状态不会自愈。

**排查**——翻 cortex 启动日志（`grep -E "对象存储|object store" <你的 cortex 日志>`），按命中行对号：

| 命中行 | 含义 | 处理 |
|---|---|---|
| `对象存储未启用(enabled=false)` | 配置没开，或**加载了错误的 config** | 开发默认读 `config/config_1.toml`，生产靠 `--config`/`CORTEX_AGENT_CONFIG` 指向 `config/config.toml`——改错文件不生效。确认实际加载那份的 `[object_storage].enabled = true` |
| `对象存储连通性自检失败(endpoint=..., bucket=...):<具体错>` | 启动时连得上配置但自检不过 | 见 Q2 三类原因（桶 / 密钥 / 端口） |
| 没有 `object store 就绪` 这行 | 初始化确实失败了 | 同上，按自检失败行的具体错处理 |

**快速排除网络/凭证**（在 cortex 所在机器执行）：

```bash
# alias set 成功 = 端口通 + 凭证对（排除网络/密钥，问题就在 bucket 或启动时序）
mc alias set cortex http://127.0.0.1:9000 '<RUSTFS_ACCESS_KEY>' '<RUSTFS_SECRET_KEY>'
# 看桶建了没
mc ls cortex/
```
...
1. 确保 RustFS 在跑 + `cortex` 桶已建（`mc ls cortex/` 没有 → `mc mb cortex/cortex`，见 §4.2）；
2. **重启 cortex**，让它重新自检——**这是关键**。重启后启动日志应出现：
   ```
   [infra] object store 就绪:endpoint=http://localhost:9000 bucket=cortex path_style=true
   ```
   再上传即恢复正常。
