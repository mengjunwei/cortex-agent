...
| 隔离 | 进程内 FFI，崩溃会带崩主进程 | AST 沙箱 + 引擎操作上限，进程内 `spawn_blocking` + 超时执行（不阻塞 runtime） |
...
GraphQL（POST /api/graphql）──►  PluginManager（RwLock<HashMap>）
  ...
  ── 同时被 validate_monitor_plugin FunctionTool 调用 ──
            ...
            L1 进程内编译        ──► 毫秒级语法检查
            L2 进程内执行        ──► spawn_blocking + 超时跑 mock 用例
...
| `rhai-runner` 二进制 | [rhai_runner.rs](file:///d:/code/rust/cortex-agent/src/bin/rhai_runner.rs) | 独立子进程 runner（**历史路径**：L2 改进程内后无生产调用方，保留备用，见 §七） |
| `SandboxVerifier` | [sandbox.rs](file:///d:/code/rust/cortex-agent/src/infra/sandbox.rs) | adk-sandbox 封装（**历史路径**：同上，仅测试引用） |
| `CodeVerifier` | [code_exec.rs](file:///d:/code/rust/cortex-agent/src/infra/code_exec.rs) | L3 验证层：封装 adk-code |
| `validate_monitor_plugin` | [validate.rs](file:///d:/code/rust/cortex-agent/src/tools/monitor_plugin/validate.rs) | LLM 自检工具，三层串联（L1/L2 进程内 + L3 子进程） |
| `PluginStore` | [plugin_store.rs](file:///d:/code/rust/cortex-agent/src/monitor/plugin_store.rs) | 插件 + 版本历史的 PostgreSQL 持久化（启动时恢复） |
| GraphQL API | [server/graphql.rs](file:///d:/code/rust/cortex-agent/src/server/graphql.rs) + [server/monitor.rs](file:///d:/code/rust/cortex-agent/src/server/monitor.rs) + [server/mod.rs](file:///d:/code/rust/cortex-agent/src/server/mod.rs) | 监控插件的注册 / 回滚 / OID 准备 / 采集值解析统一经 GraphQL（`POST /api/graphql`）暴露；resolver 在 `graphql.rs`，业务函数在 `server/monitor.rs`，`monitor_get_oids` / `monitor_calculate` 在 `server/mod.rs` |

### 安全限制

每个 Rhai Engine 都会通过 [`apply_safety_limits`](file:///d:/code/rust/cortex-agent/src/monitor/rhai_plugin.rs#L126-L135) 设置操作上限，防止 LLM 生成的恶意/错误脚本拖垮进程（L1/L2 进程内与历史子进程路径共用同一函数，防规则漂移）：
...
- **作用**：拦截语法错误与脚本内未定义函数的调用；未注册 host function 的调用属运行期错误，由 L2 执行暴露。
- **耗时**：通常 < 5ms。
- **失败行为**：直接终止该测试用例，不进入 L2。

### Layer 2：进程内受控执行（毫秒级）

通过 [`validate.rs`](file:///d:/code/rust/cortex-agent/src/tools/monitor_plugin/validate.rs) 的 `run_in_process`（`spawn_blocking` + 超时）在阻塞线程上执行 mock 用例：

1. 构建 Engine（`apply_safety_limits` + `register_host_functions`）→ 编译 AST → `call_fn` 调用顶层函数；
2. `max_operations = 1,000` 防死循环，超限由 Rhai 引擎主动终止；
3. `spawn_blocking` + `tokio::time::timeout` 兜底，panic / 死循环不会阻塞 async runtime，也不会拖垮主进程。

**为什么改进程内？** 原方案用 adk-sandbox spawn `rhai-runner` 子进程隔离，但其 stdin 管道在 Windows 上存在句柄泄漏——首次执行后子进程 `read_to_string` 永久阻塞，触发 10s 超时。Rhai 引擎自带操作上限，配合 `spawn_blocking` + 超时已足够稳定，且省去子进程定位 / spawn 开销。`rhai-runner` 与 `SandboxVerifier` 保留但当前无生产调用方（§七）。

**为什么需要 L2？** L1 只能发现语法错误，无法发现运行时错误（例如 `unwrap()` 在 None 上 panic、数组越界、除零等）。L2 实际执行用例，把这些运行时错误捕获为用例失败。
...
监控插件的注册 / 注销 / 查询 / 回滚 / OID 准备 / 采集值解析**统一通过 GraphQL 单入口** `POST /api/graphql` 暴露（详见 [API 文档](./api.md)）。所有返回值均为统一信封 `{ code, message, data }`（`code == 0` 成功，HTTP 恒 200，**无 422**）。**多用户（auth 启用）模式下注册 / 注销 / 回滚仅管理员可操作**（监控插件是全局运行时配置）；单用户（no-auth）模式放行。

> **plugin_id 规范化**：传入的 `plugin_id` 若**不是 UUID v7 格式**，注册时会被自动生成的新 UUID v7 替换（`manager.rs::normalize_plugin_id`），响应返回**最终 id**——调用方必须以响应为准做后续引用（日志会同时记录 `registered <final_id> (requested: <requested_id>)`）。传入 UUID v7 则原样沿用，可实现「覆盖更新同一插件」。
...
```json
{
  "plugin_id": "01912345-6789-7abc-def0-123456789abc",
...
**data**（成功）:
```json
{ "plugin_id": "01912345-6789-7abc-def0-123456789abc", "version": 2 }
```

**失败**（Rhai 编译失败）：返回 `code` 非 0（`BUSINESS` 2001），`message` 形如 `rhai 编译失败 (plugin_id=...): Parse error: ...`。

> 编译失败时，已存在的同名插件**保持原样不替换**。脚本上限 64 KB，`plugin_id` 入参仅允许 `[a-zA-Z0-9_-]`（1-64 字符，格式校验），但持久化 id 一律规范化为 UUID v7（见上方说明）。

### 6.2 Mutation `unregisterMonitorPlugin`

注销插件。参数 `pluginId`。**data 为 `{"removed": bool}`**——插件不存在时 `removed=false`，不报错。
...
## 七、`rhai-runner` 二进制（历史路径，保留备用）

> **状态**：L2 验证已改为进程内执行（§二），`rhai-runner` 与 `SandboxVerifier` 当前**无生产调用方**，仅 `infra::sandbox` 单元测试引用。保留原因：子进程隔离方案在需要更强隔离（如未来加载不可信第三方脚本）时可随时启用。以下各节描述其既有协议，供恢复使用时参考。

### 7.1 用途

`rhai-runner` 是本项目的附属二进制（`Cargo.toml` 的 `[[bin]]` 声明），编译产物 `rhai-runner.exe`（Windows）/ `rhai-runner`（Unix）。主进程通过 adk-sandbox 的 `ProcessBackend` 以 `Language::Command` 方式 spawn 它，在独立子进程内运行 LLM 生成的 Rhai 脚本。
...
四步任一命中即返回；全部未命中返回错误（该路径当前无生产调用方，仅测试覆盖）。
...
### 8.1 adk-sandbox（历史 L2 路径，现仅 `rhai-runner` 测试引用）
...
LLM 在对话中生成监控插件的完整工作流（monitor_plugin 内置助手已暂下线，但工具链保留——恢复 seed 或经自定义助手挂载 `validate_monitor_plugin` / `register_monitor_plugin` 工具即可复用本流程）：
...
LLM 调用 validate_monitor_plugin 工具
   ...
   └── 断言通过？── no ──► LLM 修正脚本，重试
        ...
        yes
         │
         ▼
LLM 调用 register_monitor_plugin 工具注册最终脚本
（FunctionTool，内部走 PluginManager，等价 GraphQL registerMonitorPlugin；
 插件 id 交由平台规范化为 UUID v7，工具返回最终 id）
   │
   ▼
...
本项目为 Rhai 插件系统提供了完整的单元测试 + 集成测试，运行 `cargo test --lib` 可全部执行（L1/L2 已进程内，无需预编译 runner；仅 `infra::sandbox` 的历史子进程路径测试需先 `cargo build --bin rhai-runner`）。
...
| `infra::sandbox` | 2 | adk-sandbox 子进程隔离执行（历史 L2 路径） |
| `tools::monitor_plugin::validate` | 5 | 三层串联 / 语法错误 / 断言失败 / 参数解析 |
...
```bash
# 运行全部 lib 测试（L2 为进程内执行，无外部依赖）
cargo test --lib

# 仅历史子进程路径测试需先编译 rhai-runner
cargo build --bin rhai-runner
...
3. 注册 `get_bool` 函数；
4. **单一注册点**：L1/L2 均进程内直接调用 `host_fns.rs` 的 `register_host_functions`，改一处即生效；历史子进程路径（`rhai-runner`）同样 `use` 主 crate 的同一函数（见 `src/bin/rhai_runner.rs`），从设计上杜绝双注册漂移；
5. 更新 [monitor_plugin.rs](file:///d:/code/rust/cortex-agent/src/tools/monitor_plugin/mod.rs) 的 system prompt host function 白名单表格；
6. 新增单元测试覆盖。

### 11.2 调试技巧

- **脚本运行时报错**：看 L2 用例返回的错误信息（进程内执行，panic / 运行期错误直接进结果；历史子进程路径的 `stderr` 透传仅 `infra::sandbox` 测试涉及）；
...
|------|---------|
| L1 进程内编译 | 1-5 ms |
| L2 进程内 prepare_oids / parse | 1-10 ms |
| （历史）L2 子进程 prepare_oids / parse | 80-150 ms |
