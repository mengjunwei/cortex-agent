...
麒麟自带的 Node 常是 v12 / v16，而 Playwright 要求 ≥ 20（**注意别再用 Node 20——它已于 2026-04 EOL**，选当前活跃 LTS，如 Node 22/24）。最省事的方式：**下载预编译包覆盖系统 node**，不依赖 nvm、无需编译。

```bash
# 版本号以镜像目录实际列出的为准：https://npmmirror.com/mirrors/node/
NODE_VER=v22.19.0
...
cd /usr/local
curl -L -o node.tar.xz https://npmmirror.com/mirrors/node/${NODE_VER}/node-${NODE_VER}-linux-${ARCH}.tar.xz
tar -xf node.tar.xz
ln -sf /usr/local/node-${NODE_VER}-linux-${ARCH}/bin/node /usr/local/bin/node
ln -sf /usr/local/node-${NODE_VER}-linux-${ARCH}/bin/npm  /usr/local/bin/npm
ln -sf /usr/local/node-${NODE_VER}-linux-${ARCH}/bin/npx  /usr/local/bin/npx
...
Environment=PLAYWRIGHT_MCP_EXECUTABLE_PATH=/root/.cache/ms-playwright/chromium-<revision>/chrome-linux64/chrome
...
**方式 A：配置驱动**（`config/config_1.toml` 或你实际加载的 config 文件，启动自动 upsert；当前仓库配置未预置任何 `[[mcp.seeds]]`，以下是示例——配置了 seeds 启动时会覆盖 name/endpoint/args/transport/超时等字段，env/headers 不受影响）：
...
args = "[]"
# 鉴权头不能写进 seed（McpSeedConfig 无 headers 字段，写了会被忽略）——
# 前置 nginx Basic Auth 时，装好后经「MCP 服务」界面编辑补配 headers
```
...
| `install-browser` 报 404 `NoSuchKey`（下载 `151.0.7922.x` 的 chrome 失败）| `@playwright/mcp@latest`（含全部 `0.0.x`）绑的是 **playwright alpha**，其浏览器 revision **旧镜像（npmmirror.com/mirrors/playwright）未收录**（新镜像 `cdn.npmmirror.com/binaries/playwright` 已收录大部分 CFT build，§4 已改用新镜像，通常可直接下）；仍 404 时只有官方 CDN（国内极慢）。最省事解法：复用 stable playwright 已下好的 chromium，用 `--executable-path` 指过去绕过版本检查——`CHROME=$(find /root/.cache/ms-playwright/chromium-* -name chrome -type f \| head -1)`，启动参数加 `--executable-path "$CHROME"`（等价 env `PLAYWRIGHT_MCP_EXECUTABLE_PATH`） |
...
`--host 0.0.0.0` 且**无内置鉴权**，切勿直接暴露公网。远程使用建议：防火墙限定来源 IP，或前置 nginx 加 Basic Auth（鉴权头经 cortex「MCP 服务」界面的 headers 字段配置；`[[mcp.seeds]]` 种子不含 headers，需界面补配）。
