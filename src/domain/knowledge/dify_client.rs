...
impl DifyClient {
    ...
    async fn get_retrieval_model(&self) -> Result<serde_json::Value, AppError> {
        ...
        // 降 debug:TTL 仅 10s,连续检索时每 10s 刷一条,纯缓存刷新无事件语义
        tracing::debug!(
    ...
    /// 通过文件创建文档 — 把原始文件交给 Dify，由其完成解析/切片/Embedding。
    ///
    /// Dify 自带文档解析（PDF/Word/Excel/...），无需本服务 markitdown 预处理。
    /// 端点：`/datasets/{id}/document/create-by-file`（multipart/form-data）：
    /// - `file`：文件本体
    /// - `data`：JSON 字符串（name / indexing_technique / process_rule，与 create_by_text 一致）
    pub async fn create_document_by_file(
        ...
        let data_json = serde_json::to_string(&data).map_err(|e| {
            AppError::SerializationError(format!("序列化 Dify 文件上传 data 失败: {}", e))
        })?;
        ...
        tokio::task::spawn_blocking(move || {
            ...
            let file_part = Part::bytes(&bytes)
                .file_name(&file_name)
                .mime_str(&mime)
                .map_err(|e| AppError::NetworkError(format!("Dify multipart mime 失败: {}", e)))?;
            let form = Form::new().text("data", &data_json).part("file", file_part);
            let mut resp = agent
                .post(&url)
                .header("Authorization", &format!("Bearer {}", api_key))
                .send(form)
                .map_err(|e| {
                    AppError::NetworkError(format!("Dify create-by-file 请求失败: {}", e))
                })?;
            resp.body_mut()
                .read_json()
                .map_err(|e| AppError::SerializationError(format!("解析 Dify 响应失败: {}", e)))
        ...
        .await
        .map_err(|e| AppError::NetworkError(format!("Dify create-by-file 任务失败: {}", e)))?
    }
