...
impl KnowledgeManager {
    ...
    #[allow(clippy::too_many_arguments)] // 参数即 FAQ 生成所需上下文，聚合无收益
...
8. 输出必须是合法 JSON 字符串，content 内换行使用 \n 转义，不要输出 markdown 代码块"#,
        ...
        // 注意顺序：with_config 是整体替换 config，必须放在 with_response_schema
        // 之前，否则 schema 被默认 config 覆盖丢弃（结构化输出静默失效）
