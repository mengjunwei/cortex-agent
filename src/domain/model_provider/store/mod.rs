...
struct ModelRow {
    ...
    #[diesel(sql_type = sql_types::Varchar)]
    model: String,
    /// 归属用户 id（冗余=provider.user_id；''=系统/legacy）
    #[diesel(sql_type = sql_types::Varchar)]
    user_id: String,
...
/// 单列 user_id 查询行（`SELECT user_id AS uid FROM ...`）——归属反查
#[derive(Debug, Clone, QueryableByName)]
...
struct CachedModel {
    ...
    context_window: Option<i32>,
    /// 归属用户 id（来自 provider.user_id；''=系统/legacy 桶）
    user_id: String,
}
...
struct Cache {
    ...
    /// 跨用户扁平存储（model_id 全局唯一），解析时按调用者 user_id 校验归属。
    models: HashMap<String, CachedModel>,
    /// 每用户的默认 chat 模型 id：user_id -> model_id
    default_id: HashMap<String, String>,
    /// 每用户的默认 embedding 模型 id：user_id -> model_id
    embedding_default_id: HashMap<String, String>,
}
...
impl ModelProviderStore {
    ...
    pub fn key_suffix(plain: &str) -> String {
...
async fn unbind_model_references(
    ...
    let assistants_unbound =
        diesel::sql_query("UPDATE assistants SET model_id = '' WHERE model_id = ANY($1)")
...
/// 当指定用户无默认模型时，自动将其名下最早创建的已启用模型设为默认。
///
/// **按 user_id 作用域**：每个用户独立维护至多一个默认（由部分唯一索引
/// `uq_llm_models_default ON (user_id) WHERE is_default=TRUE` 约束）。
///
/// 并发安全：通过 `NOT EXISTS` 守卫 + 忽略部分唯一索引冲突，
/// 即使多个请求并发触发，也最多有一个成功设置默认，其余优雅跳过。
async fn reassign_default_if_missing(
    conn: &mut DbPooledConnection,
    user_id: &str,
) -> Result<(), AppError> {
    match diesel::sql_query(
        ...
        WHERE id = (
                ...
                WHERE m.status = 1 AND p.status = 1 AND m.user_id = $1
                ORDER BY m.created_at ASC LIMIT 1
              )
          AND NOT EXISTS (SELECT 1 FROM llm_models WHERE is_default = TRUE AND user_id = $1)
        "#,
    )
...
/// 把默认模型从 `exclude_id` 转移给同一 `user_id` 下的另一个已启用模型（模型+供应商均启用）。
///
/// **按 user_id 作用域**：候选仅在该用户名下查找，保证转移不跨用户。
///
...
async fn reassign_default_to_any_enabled(
    ...
    let candidates = diesel::sql_query(
        ...
        INNER JOIN llm_providers p ON p.id = m.provider_id
        WHERE m.id <> $1 AND m.status = 1 AND p.status = 1 AND m.user_id = $3
...
/// 当指定用户无默认向量模型时，自动将其名下最早创建的已启用向量模型设为默认向量。
/// 对称 [`reassign_default_if_missing`]，候选额外要求 `tags` 含 `embedding`。
async fn reassign_embedding_default_if_missing(
    conn: &mut DbPooledConnection,
    user_id: &str,
) -> Result<(), AppError> {
    match diesel::sql_query(
        r#"
        UPDATE llm_models SET embedding_default = TRUE, updated_at = NOW()
        WHERE id = (
                SELECT m.id FROM llm_models m
                INNER JOIN llm_providers p ON p.id = m.provider_id
                WHERE m.status = 1 AND p.status = 1 AND m.user_id = $1
                  AND m.tags::jsonb @> '["embedding"]'
                ORDER BY m.created_at ASC LIMIT 1
              )
          AND NOT EXISTS (SELECT 1 FROM llm_models WHERE embedding_default = TRUE AND user_id = $1)
        "#,
    )
    .bind::<sql_types::Text, _>(user_id)
    .execute(conn)
    .await
    {
        Ok(_) => Ok(()),
        Err(e) if is_unique_violation(&e) => {
            tracing::debug!("[ModelProvider] 默认向量模型已被并发请求设置，跳过");
            Ok(())
        }
        Err(e) => Err(AppError::from(e)),
    }
...
/// 把默认向量模型从 `exclude_id` 转移给同一 `user_id` 下另一个已启用的向量模型。
/// 对称 [`reassign_default_to_any_enabled`]，候选额外要求 `tags` 含 `embedding`。
async fn reassign_embedding_default_to_any_enabled(
    conn: &mut DbPooledConnection,
    exclude_id: &str,
    exclude_provider_id: Option<&str>,
    user_id: &str,
    ...
    // 两步切换（先清旧再立新），与 reassign_default_to_any_enabled 一致；
    // 不用显式事务：若第二步命中唯一冲突（并发抢先），保留第一步清除结果更安全
    // （宁可暂时无默认向量，也不让被禁用模型残留为默认向量）。
    diesel::sql_query("UPDATE llm_models SET embedding_default = FALSE WHERE id = $1")
        .bind::<sql_types::Text, _>(exclude_id)
        .execute(conn)
        .await?;
    match diesel::sql_query(
        "UPDATE llm_models SET embedding_default = TRUE, updated_at = NOW() WHERE id = $1",
    )
    .bind::<sql_types::Text, _>(candidate_id)
    .execute(conn)
    .await
