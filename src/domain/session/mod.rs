...
//!     token_total     BIGINT      NOT NULL DEFAULT 0,   -- 会话级 token 用量累计峰值（持久化，重进会话即恢复）
//!     token_threshold BIGINT      NOT NULL DEFAULT 0    -- 压缩阈值（随上下文窗口计算）
//! );
//! ```
//!
...
impl SessionSettingsStore {
    ...
    /// 定时任务会话请用 [`init_scheduled_session`]（额外写 source_type/schedule_task_id）。
        ...
        diesel::sql_query(
            ...
            ON CONFLICT (session_id) DO UPDATE
                SET title        = EXCLUDED.title,
                    ...
                    updated_at   = NOW()
                WHERE session_settings.user_id = EXCLUDED.user_id
            "#,
        )
    ...
    /// 创建定时任务会话：在 init_session 基础上额外写 source_type=1 + schedule_task_id。
    /// 普通会话列表（list_page 过滤 source_type=0）不显示，仅从任务详情进入。
    pub async fn init_scheduled_session(
        &self,
        session_id: &str,
        user_id: &str,
        title: &str,
        ...
        diesel::sql_query(
            ...
            INSERT INTO session_settings
                (session_id, user_id, title, agent_type, assistant_id, source_type, schedule_task_id, trigger_kind, updated_at)
        ...
        .bind::<sql_types::Text, _>(agent_type)
        .bind::<sql_types::Nullable<sql_types::Text>, _>(assistant_id)
        .bind::<sql_types::Text, _>(schedule_task_id)
        .bind::<sql_types::Varchar, _>(trigger_kind)
        .execute(&mut conn)
        .await?;
        Ok(())
    }

    /// 定时任务运行历史：按 schedule_task_id 查近 30 天的会话（任务详情页用）。
    pub async fn list_scheduled_runs(
        &self,
        schedule_task_id: &str,
    ) -> Result<Vec<ScheduledRunRow>, AppError> {
        let mut conn = self.get_conn().await?;
        let rows = diesel::sql_query(
            r#"
            SELECT session_id, title, trigger_kind, token_total, updated_at
        ...
        .bind::<sql_types::Text, _>(schedule_task_id)
        .get_results::<ScheduledRunRow>(&mut conn)
        .await?;
        Ok(rows)
    }

    /// 分页查某任务近 30 天运行记录。返回 (当前页, 总条数)。
    pub async fn list_scheduled_runs_paged(
        &self,
        schedule_task_id: &str,
        page: i64,
        page_size: i64,
    ) -> Result<(Vec<ScheduledRunRow>, i64), AppError> {
        let mut conn = self.get_conn().await?;
        let cnt: CountRow = diesel::sql_query(
            "SELECT COUNT(*) AS cnt FROM session_settings \
             WHERE schedule_task_id = $1 AND updated_at >= NOW() - INTERVAL '30 days'",
        )
        .bind::<sql_types::Text, _>(schedule_task_id)
        .get_result(&mut conn)
        .await?;
    ...
    /// 清理某任务超过 30 天的定时会话行（每次运行成功后顺手清理；返回被删 session_id 供级联删 adk 会话）。
    pub async fn delete_scheduled_older_than_30d(
        &self,
        schedule_task_id: &str,
        ...
        .bind::<sql_types::Text, _>(schedule_task_id)
        .get_results::<SessionIdRow>(&mut conn)
        .await?;
    ...
    /// 写入/更新会话级 token 用量快照（对齐 codex 会话级 `token_info` 持久化）。
    /// 每轮 run 结束落库：进程重启后重进会话，前端可立即恢复「已用 / 阈值」显示，
    /// 后端亦可据此恢复内存累计基线（见 `create_event_stream` 播种），无需等首条新消息。
    pub async fn set_token_usage(
        ...
        diesel::sql_query(
            r#"
            INSERT INTO session_settings (session_id, token_total, token_threshold, updated_at)
            VALUES ($1, $2, $3, NOW())
            ON CONFLICT (session_id) DO UPDATE
                SET token_total     = EXCLUDED.token_total,
                    token_threshold = EXCLUDED.token_threshold,
                    updated_at      = NOW()
            "#,
        )
        .bind::<sql_types::Text, _>(session_id)
        .bind::<sql_types::BigInt, _>(total as i64)
        .bind::<sql_types::BigInt, _>(threshold as i64)
        .execute(&mut conn)
        .await?;
        Ok(())
    }
    ...
    pub async fn get_model(&self, session_id: &str) -> Result<Option<String>, AppError> {
        let mut conn = self.get_conn().await?;
        let rows =
            diesel::sql_query("SELECT model_id AS val FROM session_settings WHERE session_id = $1")
                .bind::<sql_types::Text, _>(session_id)
    ...
    pub async fn get_permission_policy(        &self,
        session_id: &str,
    ...
    /// 读取会话归属来源（source_type / schedule_task_id / title），供前端识别定时任务会话并只读。
    /// title 一并返回：定时任务会话不在普通会话列表（source_type=0 过滤），前端列表取不到标题，需此处兜底。
    /// 返回 (source_type, schedule_task_id, title)；会话不存在 → None。
    pub async fn get_source_info(
        &self,
        session_id: &str,
    ) -> Result<Option<(i16, Option<String>, Option<String>)>, AppError> {
        let mut conn = self.get_conn().await?;
        let rows = diesel::sql_query(
            "SELECT source_type AS st, schedule_task_id AS tid, title AS t FROM session_settings WHERE session_id = $1",
        )
        .bind::<sql_types::Varchar, _>(session_id)
        .get_results::<SourceInfoRow>(&mut conn)
        .await?;
        Ok(rows.into_iter().next().map(|r| (r.st, r.tid, r.t)))
    }

    /// 读取会话级 token 用量快照（None=无记录或会话不存在）。
    /// 用于重进会话时立即恢复前端显示与后端累计基线。
    pub async fn get_token_usage(&self, session_id: &str) -> Result<Option<(u64, u64)>, AppError> {
        let mut conn = self.get_conn().await?;
        let rows = diesel::sql_query(
            "SELECT token_total AS total, token_threshold AS threshold \
             FROM session_settings WHERE session_id = $1",
        )
        .bind::<sql_types::Text, _>(session_id)
        .get_results::<TokenUsageRow>(&mut conn)
        .await?;
        Ok(rows
            .into_iter()
            .next()
            .map(|r| (r.total as u64, r.threshold as u64)))
    }
    ...
    pub async fn get_owner(&self, session_id: &str) -> Result<Option<String>, AppError> {
        let mut conn = self.get_conn().await?;
        let rows =
            diesel::sql_query("SELECT user_id AS val FROM session_settings WHERE session_id = $1")
                .bind::<sql_types::Text, _>(session_id)
                .get_results::<NullableTextRow>(&mut conn)
                .await?;
        Ok(rows
    ...
    pub async fn list_page(
        ...
        // 始终过滤定时任务会话（source_type=0 才进普通会话列表；定时会话从任务详情进入）。
        let mut where_sql = if admin_view {
            String::from("COALESCE(s.source_type, 0) = 0")
        } else {
            String::from("s.user_id = $1 AND COALESCE(s.source_type, 0) = 0")
        };
        ...
        let total = cq.get_result::<CountRow>(&mut conn).await?.cnt;
...
/// 单 session_id 列行（清理 RETURNING 用）
#[derive(Debug, Clone, QueryableByName)]
struct SessionIdRow {
    #[diesel(sql_type = sql_types::Varchar)]
    session_id: String,
...
/// 会话级 token 用量快照行（token_total / token_threshold）
#[allow(dead_code)]
#[derive(Debug, Clone, QueryableByName)]
...
/// 会话来源行（source_type / schedule_task_id / title）
#[derive(Debug, Clone, QueryableByName)]
