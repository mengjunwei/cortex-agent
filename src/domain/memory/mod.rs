...
impl MemoryStore {
    ...
    /// 管理页：列出记忆。普通用户仅自己（user_id 命中）；管理员（admin_view=true）看全部。
    pub async fn list_for_user(
        ...
        let mut conn = self.get_conn().await?;
        diesel::sql_query(format!(
            "SELECT {MEM_COLS} FROM memories WHERE ($1 OR user_id = $2) \
    ...
    /// 编辑记忆正文/类型。普通用户带 user_id 防越权；管理员（is_admin）可改任意人的记忆。
    pub async fn update(
        &self,
        id: &str,
        user_id: &str,
    ...
    /// 删除。普通用户只能删自己的；管理员（is_admin）可删任意人的记忆。
    pub async fn delete(&self, id: &str, user_id: &str, is_admin: bool) -> Result<(), AppError> {
        let mut conn = self.get_conn().await?;
        diesel::sql_query("DELETE FROM memories WHERE id = $1 AND ($2 OR user_id = $3)")
            .bind::<sql_types::Text, _>(id)
            .bind::<sql_types::Bool, _>(is_admin)
            .bind::<sql_types::Text, _>(user_id)
            .execute(&mut conn)
...
impl MemoryProposalStore {
    ...
    /// 卡片列表：用户的待确认建议；管理员（admin_view=true）看全部用户的待确认。
    pub async fn list_pending(
        ...
        diesel::sql_query(format!(
            "SELECT {PROP_COLS} FROM memory_proposals \
             WHERE status = 0 AND ($1 OR user_id = $2) \
    ...
    /// 乐观锁领取：只把「pending」的建议置为 accepted，返回领取到的建议。
    /// 并发安全（同一建议只会被领取一次）；普通用户仅本人 + pending；管理员（is_admin）可领取任意人。
    pub async fn claim(
        &self,
        id: &str,
        user_id: &str,
        ...
        let n = diesel::sql_query(
            "UPDATE memory_proposals SET status = 1 \
             WHERE id = $1 AND status = 0 AND ($2 OR user_id = $3)",
        )
        .bind::<sql_types::Text, _>(id)
        .bind::<sql_types::Bool, _>(is_admin)
        .bind::<sql_types::Text, _>(user_id)
    ...
    /// 乐观锁拒绝：把「pending」的建议置为 rejected。普通用户仅本人；管理员可拒绝任意。返回是否生效。
    pub async fn reject(&self, id: &str, user_id: &str, is_admin: bool) -> Result<bool, AppError> {
        let mut conn = self.get_conn().await?;
        let n = diesel::sql_query(
    ...
    pub async fn list_all_for_user(
        ...
        diesel::sql_query(format!(
            "SELECT {PROP_COLS} FROM memory_proposals \
             WHERE ($1 OR user_id = $2) ORDER BY created_at DESC LIMIT 500"
        ))
        .bind::<sql_types::Bool, _>(admin_view)
        .bind::<sql_types::Text, _>(user_id)
        .get_results::<Proposal>(&mut conn)
