...
mod penalty_forward_tests {
    ...
    fn forwards_frequency_and_presence_penalty() {
        // config 设了 penalty 时，必须透传到请求 JSON（当前缺失 → 重复退化诱因之一）
        ...
        // f32 → JSON → f64 有精度损失（0.4_f32 ≈ 0.4000000059…），用近似比较
    ...
    fn omits_penalty_when_not_set() {
        // config 未设 penalty 时，JSON 不应含该字段（不覆盖模型/API 默认）
