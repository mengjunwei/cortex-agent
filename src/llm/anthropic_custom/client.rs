    ...
    pub(super) fn build_message_params(
        ...
        let mut tools = if request.tools.is_empty() {
            ...
            if filtered_tools.is_empty() {
                ...
                // 绑定 AnthropicSchemaAdapter 构造缓存，使 SchemaCache::normalize 用对适配器
                //（取代已废弃的 get_or_normalize(&schema, adapter)）。
                static SCHEMA_CACHE: LazyLock<SchemaCache> = LazyLock::new(|| {
                    SchemaCache::for_adapter(std::sync::Arc::new(AnthropicSchemaAdapter))
                });
    ...
    async fn generate_content(
        ...
        let response_stream = try_stream! {
            if stream {
                ...
                // 折算成统一语义（input 含 cache）：见 convert::unified_input_tokens 文档注释。
                //
                // 快照存**原始字段值**（不做折算），末帧构造 UsageMetadata 时才统一折算。
                // 原因：MessageDelta 的 usage 是 cumulative 且各字段独立 Option——某字段
                // None 表示「本帧未提供」而非「为零」，须沿用快照（若把 None 当 0 参与
                // 折算，末条 delta 只回填 input 时会把首帧 cache_read 丢掉，prompt 与
                // cache_read 失配，下游净口径 effective = total − cache_read 直接归零）。
                let mut usage_input_tokens: i32 = 0;
                let mut usage_cache_read: Option<i32> = None;
                let mut usage_cache_creation: Option<i32> = None;
                let mut usage_cache_creation_1h: Option<i32> = None;
                ...
                while let Some(event_result) = pinned_stream.next().await {
                    ...
                    match event {
                        ...
                        MessageStreamEvent::MessageDelta(delta_event) => {
                            // MessageDelta usage 是 cumulative：各 Option 字段 Some 时覆盖
                            // 快照、None 沿用（「未提供」≠「为零」，见上方快照注释）。
                            if let Some(v) = delta_event.usage.input_tokens {
                                usage_input_tokens = v;
                            }
                            if let Some(v) = delta_event.usage.cache_read_input_tokens {
                                usage_cache_read = Some(v);
                            }
                            if let Some(v) = delta_event.usage.cache_creation_input_tokens {
                                usage_cache_creation = Some(v);
                            }
                            // MessageDeltaUsage 无 _1h 字段，沿用快照。
                            ...
                            if let Some(stop_reason) = &delta_event.delta.stop_reason {
                                ...
                                // 终帧统一折算：从原始字段快照合成统一语义 UsageMetadata
                                //（input 含 cache，见 convert::unified_input_tokens）。
                                let usage_input = convert::merge_unified_input(
                                    usage_input_tokens,
                                    usage_cache_read,
                                    usage_cache_creation,
                                    usage_cache_creation_1h,
                                );
                                let usage_metadata = adk_rust::UsageMetadata {
                                    prompt_token_count: usage_input,
                                    candidates_token_count: delta_event.usage.output_tokens,
                                    total_token_count: usage_input
                                        .saturating_add(delta_event.usage.output_tokens),
                                    cache_read_input_token_count: usage_cache_read,
                                    cache_creation_input_token_count: usage_cache_creation,
                                    ..Default::default()
                                };
                        ...
                        MessageStreamEvent::MessageStart(start_event) => {
                            ...
                            // Store raw usage fields for the final UsageMetadata
                            // （末帧统一折算，见上方快照注释）。
                            usage_input_tokens = start_event.message.usage.input_tokens;
                            // Store cache token counts for propagation to the final MessageDelta
                            usage_cache_read = start_event.message.usage.cache_read_input_tokens;
                            usage_cache_creation = start_event.message.usage.cache_creation_input_tokens;
                            usage_cache_creation_1h = start_event.message.usage.cache_creation_input_tokens_1h;
                ...
                // 转换一次复用结果（旧实现调用两次 from_anthropic_message、第一次整体
                // 丢弃，白白双跑 content 遍历 + 深拷贝；cache 用量已在 usage_metadata
                // 的 cache_read/cache_creation 字段里，元组第二项已随之移除）。
