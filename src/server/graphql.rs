...
/// 从 GraphQL Context 取出当前用户是否管理员（graphql_handler 注入 `is_admin: bool`）。
///
...
fn current_is_admin(ctx: &Context<'_>) -> bool {
    ctx.data::<bool>().copied().unwrap_or(false)
}

/// 删除类操作的权限守卫：**API Token 认证的请求仅允许删除会话**，其他删除一律拒绝。
///
/// 判定依据：`graphql_handler` 注入的 `via_api_token: bool`（请求通过 Authorization: Bearer
/// 成功认证 = 账户 API Token 程序化访问）。账号登录（Cookie JWT）与未登录不受此限。
/// 返回 `Some(Json)` 表示被拒，resolver 应直接 `return` 该响应。
fn reject_api_token_delete(ctx: &Context<'_>) -> Option<Json> {
    let via_api_token = ctx.data::<bool>().copied().unwrap_or(false);
    if via_api_token {
...
impl QueryRoot {
    ...
    async fn models(&self, ctx: &Context<'_>) -> Json {
        Json(super::models(state_of(ctx)).await)
    }
    ...
    async fn assistants(&self, ctx: &Context<'_>) -> Json {
        Json(super::assistant::list_assistants(state_of(ctx)).await)
    }

    /// 单个助手详情
    async fn assistant(&self, ctx: &Context<'_>, id: String) -> Json {
        Json(super::assistant::get_assistant(state_of(ctx), &id).await)
    }
    ...
    async fn session_thinking_level(&self, ctx: &Context<'_>, id: String) -> Json {
        Json(super::session::get_session_thinking_level(state_of(ctx), &id).await)
    }

    /// 会话级审批方式（沙箱模式 + 审批策略，未设置 → 全局 [shell] 默认）
    async fn session_permission_policy(&self, ctx: &Context<'_>, id: String) -> Json {
        Json(super::session::get_session_permission_policy(state_of(ctx), &id).await)
    }
    ...
    async fn memories(&self, ctx: &Context<'_>) -> Json {
        Json(super::memory::list_memories(state_of(ctx), &current_user_id(ctx)).await)
    }

    /// 当前用户的待确认记忆建议（卡片）
    async fn memory_proposals(&self, ctx: &Context<'_>) -> Json {
        Json(super::memory::list_memory_proposals(state_of(ctx), &current_user_id(ctx)).await)
    }

    // -------- 知识库 --------

    /// 知识库实例列表（多 provider：Dify 外挂 + 内置）
    async fn kb_instances(&self, ctx: &Context<'_>) -> Json {
        Json(super::knowledge_instances::kb_instance_list(state_of(ctx)).await)
    }
    ...
    async fn kb_instance_documents(&self, ctx: &Context<'_>, input: Json) -> Json {
        ...
        Json(super::knowledge::kb_instance_documents(state_of(ctx), params).await)
    }
    ...
    async fn kb_instance_segments(
        ...
        Json(super::knowledge::kb_instance_segments(state_of(ctx), &instance_id, &doc_id).await)
    }
    ...
    async fn device_search(&self, ctx: &Context<'_>, input: Json) -> Json {
        ...
        Json(super::device::device_search(state_of(ctx), req).await)
    }
    ...
    async fn model_providers(&self, ctx: &Context<'_>) -> Json {
        Json(super::model_provider::list_providers(state_of(ctx)).await)
    }
...
impl MutationRoot {
    ...
    async fn rename_session(&self, ctx: &Context<'_>, id: String, title: String) -> Json {
        Json(super::session::rename_session(state_of(ctx), &id, &title).await)
    }
    ...
    async fn update_session_thinking_level(
        ...
        Json(super::session::update_session_thinking_level(state_of(ctx), &id, &level).await)
    }
    ...
    async fn update_session_permission_policy(
        ...
        if sandbox_mode.trim() == crate::domain::permissions::SandboxMode::DangerFullAccess.codex_id()
            && !current_is_admin(ctx)
        {
    ...
    async fn update_memory(&self, ctx: &Context<'_>, id: String, input: Json) -> Json {
        Json(super::memory::update_memory(state_of(ctx), &current_user_id(ctx), id, input.0).await)
    }

    /// 删除记忆
    async fn delete_memory(&self, ctx: &Context<'_>, id: String) -> Json {
        Json(super::memory::delete_memory(state_of(ctx), &current_user_id(ctx), id).await)
    }

    /// 采纳记忆建议（转正写入 memories）
    async fn accept_memory_proposal(&self, ctx: &Context<'_>, id: String) -> Json {
        Json(super::memory::accept_memory_proposal(state_of(ctx), &current_user_id(ctx), id).await)
    }

    /// 忽略记忆建议
    async fn reject_memory_proposal(&self, ctx: &Context<'_>, id: String) -> Json {
        Json(super::memory::reject_memory_proposal(state_of(ctx), &current_user_id(ctx), id).await)
    }
    ...
    async fn cancel_run(&self, ctx: &Context<'_>, thread_id: String) -> Json {
        Json(super::sse::cancel(state_of(ctx), &thread_id).await)
    }
    ...
    async fn create_assistant(&self, ctx: &Context<'_>, input: Json) -> Json {
        Json(super::assistant::create_assistant(state_of(ctx), &input.0).await)
    }

    /// AI 智能生成助手草稿（不落库，只返回四字段供前端填充表单）
    async fn generate_assistant(&self, ctx: &Context<'_>, input: Json) -> Json {
        Json(super::assistant::generate_assistant(state_of(ctx), &input.0).await)
    }

    /// 更新自定义助手
    async fn update_assistant(&self, ctx: &Context<'_>, id: String, input: Json) -> Json {
        Json(super::assistant::update_assistant(state_of(ctx), &id, &input.0).await)
    }
    ...
    async fn delete_assistant(&self, ctx: &Context<'_>, id: String, force: Option<bool>) -> Json {
        ...
        Json(super::assistant::delete_assistant(state_of(ctx), &id, force.unwrap_or(false)).await)
    }

    /// 复制助手为自定义副本
    async fn duplicate_assistant(&self, ctx: &Context<'_>, id: String) -> Json {
        Json(super::assistant::duplicate_assistant(state_of(ctx), &id).await)
    }

    /// 生成/续用分享口令
    async fn share_assistant(&self, ctx: &Context<'_>, id: String) -> Json {
        Json(super::assistant::share_enable(state_of(ctx), &id).await)
    }

    /// 关闭分享口令
    async fn unshare_assistant(&self, ctx: &Context<'_>, id: String) -> Json {
        Json(super::assistant::share_disable(state_of(ctx), &id).await)
    }

    /// Fork 公开/分享助手到本地
    async fn fork_assistant(&self, ctx: &Context<'_>, id: String) -> Json {
        Json(super::assistant::fork_assistant(state_of(ctx), &id).await)
    }
    ...
    async fn bind_assistant_kb_instance(
        ...
        let kb = kb_instance_id
            ...
            .filter(|s| !s.is_empty());
        Json(super::assistant::set_kb_instance(state_of(ctx), &assistant_id, kb.as_deref()).await)
    }

    /// 导入助手 JSON
    async fn import_assistant(&self, ctx: &Context<'_>, input: Json) -> Json {
        Json(super::assistant::import_one(state_of(ctx), &input.0).await)
    }

    /// 导出助手为 JSON
    async fn export_assistant(&self, ctx: &Context<'_>, id: String) -> Json {
        Json(super::assistant::export_one(state_of(ctx), &id).await)
    }
    ...
    async fn kb_learn(&self, ctx: &Context<'_>, input: Json) -> Json {
        ...
        Json(super::knowledge::kb_learn(state_of(ctx), &current_user_id(ctx), req).await)
    }
    ...
    async fn kb_learn_regenerate(&self, ctx: &Context<'_>, input: Json) -> Json {
        ...
        Json(super::knowledge::kb_learn_regenerate(state_of(ctx), &current_user_id(ctx), req).await)
    }
    ...
    async fn kb_learn_commit(&self, ctx: &Context<'_>, input: Json) -> Json {
        ...
        Json(super::knowledge::kb_learn_commit(state_of(ctx), req).await)
    }
    ...
    async fn kb_instance_create(&self, ctx: &Context<'_>, input: Json) -> Json {
        ...
        Json(super::knowledge_instances::kb_instance_create(state_of(ctx), req).await)
    }

    /// 更新知识库实例（secret 字段留空保留原值）
    async fn kb_instance_update(&self, ctx: &Context<'_>, input: Json) -> Json {
        ...
        Json(super::knowledge_instances::kb_instance_update(state_of(ctx), req).await)
    }

    /// 删除知识库实例
    /// 删除知识库实例（force 省略/false=仅预检返回影响清单；force=true=执行解绑+删除）
    async fn kb_instance_delete(&self, ctx: &Context<'_>, id: String, force: Option<bool>) -> Json {
        if let Some(denied) = reject_api_token_delete(ctx) {
            return denied;
        }
        Json(
            super::knowledge_instances::kb_instance_delete(state_of(ctx), id, force.unwrap_or(false))
                .await,
        )
    }
    ...
    async fn kb_instance_test(&self, ctx: &Context<'_>, id: String) -> Json {
        Json(super::knowledge_instances::kb_instance_test(state_of(ctx), id).await)
    }
    ...
    async fn kb_instance_upload(&self, ctx: &Context<'_>, input: Json) -> Json {
        ...
        Json(super::knowledge::kb_instance_upload(state_of(ctx), req).await)
    }
    ...
    async fn kb_instance_delete_document(
        ...
        Json(
            super::knowledge::kb_instance_delete_document(state_of(ctx), &instance_id, &doc_id)
                .await,
        )
    }
    ...
    async fn create_model_provider(&self, ctx: &Context<'_>, input: Json) -> Json {
        let req: crate::model_provider::dto::CreateProviderRequest =
            match serde_json::from_value(input.0) {
                Ok(r) => r,
                Err(e) => return parse_err(e),
            };
        Json(super::model_provider::create_provider(state_of(ctx), req).await)
    }

    /// 编辑供应商（不含密钥）
    async fn update_model_provider(&self, ctx: &Context<'_>, id: String, input: Json) -> Json {
        let req: crate::model_provider::dto::UpdateProviderRequest =
            match serde_json::from_value(input.0) {
                Ok(r) => r,
                Err(e) => return parse_err(e),
            };
        Json(super::model_provider::update_provider(state_of(ctx), &id, req).await)
    }
    ...
    async fn delete_model_provider(
        ...
        Json(
            super::model_provider::delete_provider(state_of(ctx), &id, force.unwrap_or(false))
                .await,
        )
    }
    ...
    async fn reset_model_provider_key(&self, ctx: &Context<'_>, id: String, input: Json) -> Json {
        let req: crate::model_provider::dto::ResetKeyRequest = match serde_json::from_value(input.0)
        {
            Ok(r) => r,
            Err(e) => return parse_err(e),
        };
        Json(super::model_provider::reset_key(state_of(ctx), &id, req).await)
    }

    /// 新建模型
    async fn create_model(&self, ctx: &Context<'_>, provider_id: String, input: Json) -> Json {
        let req: crate::model_provider::dto::CreateModelRequest =
            match serde_json::from_value(input.0) {
                Ok(r) => r,
                Err(e) => return parse_err(e),
            };
        Json(super::model_provider::create_model(state_of(ctx), &provider_id, req).await)
    }

    /// 编辑模型
    async fn update_model(&self, ctx: &Context<'_>, id: String, input: Json) -> Json {
        let req: crate::model_provider::dto::UpdateModelRequest =
            match serde_json::from_value(input.0) {
                Ok(r) => r,
                Err(e) => return parse_err(e),
            };
        Json(super::model_provider::update_model(state_of(ctx), &id, req).await)
    }
    ...
    async fn delete_model(&self, ctx: &Context<'_>, id: String, force: Option<bool>) -> Json {
        ...
        Json(super::model_provider::delete_model(state_of(ctx), &id, force.unwrap_or(false)).await)
    }

    /// 设为默认模型
    async fn set_default_model(&self, ctx: &Context<'_>, id: String) -> Json {
        Json(super::model_provider::set_default(state_of(ctx), &id).await)
    }

    /// 设为默认 embedding 模型（知识库内置 provider 用，全局唯一）
    async fn set_embedding_default_model(&self, ctx: &Context<'_>, id: String) -> Json {
        Json(super::model_provider::set_embedding_default(state_of(ctx), &id).await)
    }
    ...
    async fn create_mcp_server(&self, ctx: &Context<'_>, input: Json) -> Json {
        ...
        Json(super::mcp::create_server(state_of(ctx), req).await)
    }
    ...
    async fn update_mcp_server(&self, ctx: &Context<'_>, id: String, input: Json) -> Json {
        ...
        Json(super::mcp::update_server(state_of(ctx), &id, req).await)
    }
    ...
    async fn delete_mcp_server(&self, ctx: &Context<'_>, id: String, force: Option<bool>) -> Json {
        ...
        Json(super::mcp::delete_server(state_of(ctx), &id, force.unwrap_or(false)).await)
    }

    /// 手动探测 MCP Server（强制重连 + 工具发现）
    async fn probe_mcp_server(&self, ctx: &Context<'_>, id: String) -> Json {
        Json(super::mcp::probe_server(state_of(ctx), &id).await)
    }

    /// 批量设置 MCP 服务状态（ids 为 null 时表示全选匹配项）
    async fn batch_set_mcp_status(&self, ctx: &Context<'_>, input: Json) -> Json {
        Json(super::mcp::batch_set_status(state_of(ctx), input.0).await)
    }

    /// 批量删除 MCP 服务（ids 为 null 时表示全选匹配项）
    async fn batch_delete_mcp_servers(&self, ctx: &Context<'_>, input: Json) -> Json {
        if let Some(denied) = reject_api_token_delete(ctx) {
            return denied;
        }
        Json(super::mcp::batch_delete_servers(state_of(ctx), input.0).await)
    }

    /// 批量探测 MCP 服务（仅支持指定 ID 列表）
    async fn batch_probe_mcp_servers(&self, ctx: &Context<'_>, input: Json) -> Json {
        Json(super::mcp::batch_probe_servers(state_of(ctx), input.0).await)
    }
