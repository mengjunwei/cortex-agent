...
pub(super) fn strip_artifact_markers(text: &str) -> String {
    ...
    // 剥标记后清理它留下的空行（标记常独占一行），避免正文多出空白
    let stripped = re.replace_all(text, "");
    // 收敛标记所在行残留的连续空行（最多保留一个换行衔接）
    stripped
        .lines()
        .map(|l| l.trim_end())
        .collect::<Vec<_>>()
...
pub(super) fn is_pure_artifact_command(args: &Value) -> bool {
    ...
    let mark_re = MARK_RE.get_or_init(|| {
        Regex::new(r"\[\[ARTIFACT:[^|\]]*\|[^|\]]*\|[^|\]]*\]\]").unwrap()
    });
