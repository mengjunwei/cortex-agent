...
/// 多模态附件描述符（image / document）
#[derive(Debug, Deserialize, Clone)]
pub struct InputAttachment {
    /// `data:<mime>;base64,...`（本地上传）或 `https://...`（对象存储 presigned URL / 外链）
    pub url: String,
    /// MIME 类型，如 `image/png`、`application/vnd.openxmlformats-officedocument.wordprocessingml.document`
    pub mime_type: String,
    /// 原始文件名（上传时由前端回填；用于文档展示与注入文本标注，可选）
    #[serde(default)]
...
pub enum SseEventMsg {
    ...
    ContextUsage {
        ...
        /// 当前上下文占用（gross，与软着陆判定同口径；压缩后自然回落）。
        /// 已 cap 到 window_size：borrow 期间真实 usage 可超窗口，前端不应看到超限值。
        total_tokens: u64,
        /// 子 agent（spawn_agent 并行任务）的本轮 token 花费；独立于 total_tokens
        /// （total 是主 agent 上下文已用量，语义不同，不合并以免进度条失真）。
        child_tokens: u64,
        /// 压缩软闸（context_window × 0.95）：到此值触发 borrow / 早压缩提醒。
        threshold: u64,
        /// 模型上下文窗口总量：前端进度条分母（已用 / 窗口总量），永远不超过 100%。
        window_size: u64,
        /// 上下文剩余百分比（对齐 codex：0-100，减 BASELINE_TOKENS 后计算，clamp 不超限）。
        context_remaining_percent: u8,
        /// 会话累计消耗（计费语义高水位，压缩清零；前端暂不渲染，预留）。
        session_total_tokens: u64,
    },
    /// 上下文已压缩（L3 intra-turn 压缩检查点）：前端可据此提示「上下文已自动整理」
    #[serde(rename = "CONTEXT_COMPACTED")]
    ContextCompacted {
        /// 会话内累计压缩次数（≥2 前端可提示「建议新建会话」；跨 run 持久）
    ...
    /// 模型经 send_user_message_async 工具发的中途用户可见消息（进度更新/阻塞提问）。
    /// 独立于 TEXT_MESSAGE_*（turn 最终答案流）：一次整条推送、非流式；前端渲染为
    /// 中途气泡，与最终回复区分。用户的回复走正常输入通道（steer 注入或下一轮）。
    #[serde(rename = "ASYNC_USER_MESSAGE")]
    AsyncUserMessage {
        /// 消息正文（模型生成，原样透传）
        message: String,
    },
