...
/// 解析 FAQ 学习的目标知识库实例：显式传则用，否则取调用者可见的第一个启用实例。
async fn resolve_kb_instance(
    state: &AppState,
    user_id: &str,
    id: Option<&str>,
) -> Result<String, Value> {
    ...
    // 默认取调用者可见（自己的或公开的）第一个启用实例
    let store = state.knowledge_manager.kb_instance_store();
    let insts = match store.list_for_owner(user_id, false).await {
        Ok(v) => v,
        Err(e) => return Err(response::err(code::DATABASE, e.to_string())),
    };
    insts
        .into_iter()
        .find(|i| i.status == 1)
        ...
        .ok_or_else(|| {
            response::err(
                code::BUSINESS,
                "没有可见的启用知识库实例，请先创建或指定 instance_id",
            )
        })
}
...
pub async fn kb_learn(
    state: &AppState,
    user_id: &str,
    is_admin: bool,
    input: KbLearnRequest,
) -> Value {
    ...
    let (model, model_name) = match resolve_kb_model(
        state,
    ...
    let instance_id = match resolve_kb_instance(state, user_id, input.instance_id.as_deref()).await
    {
        Ok(id) => id,
        Err(v) => return v,
    };
    // 候选生成需连接知识库 provider（读）：校验可读
    if let Err(v) =
        super::knowledge_instances::require_readable(state, &instance_id, user_id, is_admin).await
    {
        return v;
    }
    match state
...
pub async fn kb_learn_commit(
    state: &AppState,
    user_id: &str,
    is_admin: bool,
    input: KbCommitRequest,
) -> Value {
    ...
    // 提交写入知识库（写）：校验归属
    if let Err(v) =
        super::knowledge_instances::require_writable(state, &instance_id, user_id, is_admin).await
    {
        return v;
    }
    match state
...
pub async fn kb_instance_upload(
    state: &AppState,
    user_id: &str,
    is_admin: bool,
    input: KbInstanceDocUploadRequest,
) -> Value {
    // 上传文档（写）：校验归属
    if let Err(v) =
        super::knowledge_instances::require_writable(state, &input.instance_id, user_id, is_admin)
            .await
    {
        return v;
    }
...
pub async fn kb_instance_documents(
    state: &AppState,
    user_id: &str,
    is_admin: bool,
    params: KbInstanceDocsQuery,
) -> Value {
    // 文档列表（读）：校验可读
    if let Err(v) =
        super::knowledge_instances::require_readable(state, &params.instance_id, user_id, is_admin)
            .await
    {
        return v;
    }
...
pub async fn kb_instance_segments(
    state: &AppState,
    user_id: &str,
    is_admin: bool,
    instance_id: &str,
