...
/// 列出当前用户的全部记忆（管理页）
pub async fn list_memories(state: &AppState, user_id: &str) -> Value {
    let store = match memory_store(state) {
        Ok(s) => s,
        Err(v) => return v,
    };
    match store.list_for_user(user_id).await {
        Ok(list) => response::ok(json!({ "items": list })),
        Err(e) => response::from_app_error(&e),
    }
}

/// 列出当前用户的待确认记忆建议（卡片）
pub async fn list_memory_proposals(state: &AppState, user_id: &str) -> Value {
    let store = match proposal_store(state) {
        Ok(s) => s,
        Err(v) => return v,
    };
    match store.list_pending(user_id).await {
        Ok(list) => response::ok(json!({ "items": list })),
        Err(e) => response::from_app_error(&e),
    }
}
...
/// 编辑记忆正文/类型
pub async fn update_memory(state: &AppState, user_id: &str, id: String, body: Value) -> Value {
    let store = match memory_store(state) {
        Ok(s) => s,
        Err(v) => return v,
    };
    ...
    let mem_t = parse_mem_type(&body);
    match store.update(&id, user_id, mem_t, &content).await {
...
/// 删除记忆
pub async fn delete_memory(state: &AppState, user_id: &str, id: String) -> Value {
    let store = match memory_store(state) {
        Ok(s) => s,
        Err(v) => return v,
    };
...
/// 采纳记忆建议：claim（乐观锁 + 防越权）→ 转正写入 memories
pub async fn accept_memory_proposal(state: &AppState, user_id: &str, id: String) -> Value {
    ...
    let proposal = match p_store.claim(&id, user_id).await {
        Ok(Some(p)) => p,
...
/// 忽略记忆建议
pub async fn reject_memory_proposal(state: &AppState, user_id: &str, id: String) -> Value {
    let store = match proposal_store(state) {
        Ok(s) => s,
        Err(v) => return v,
    };
    match store.reject(&id, user_id).await {
