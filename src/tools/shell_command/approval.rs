...
impl ShellApprovalRegistry {
    ...
    /// 查询某待审批项所属会话（**不消费**），用于审批前的归属校验。
    /// None=审批不存在（已超时/已处理）；存在则返回其 session_id。
    pub async fn session_of(&self, approval_id: &str) -> Option<String> {
        let guard = self.pending.lock().await;
        guard.get(approval_id).map(|(sid, _)| sid.clone())
    }

    /// 原子「消耗 + 返回 session_id」：单次锁内 remove 并回传归属 session_id。
    ///
    /// 解决 `session_of` + `resolve` 两步锁之间的 TOCTOU 竞态：
    /// 调用方拿到 sender 后在锁外做 access check，再 send decision。
    /// access 失败时 send Rejected（条目已消耗，不可恢复——安全方向）。
    pub async fn resolve_with_session(
        &self,
        approval_id: &str,
    ) -> Option<(String, oneshot::Sender<ApprovalDecision>)> {
        let mut guard = self.pending.lock().await;
        guard.remove(approval_id)
    }
