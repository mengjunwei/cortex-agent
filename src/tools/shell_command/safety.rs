...
pub fn classify(command: &str) -> Safety {
    ...
    for sub in split_compound(trimmed) {
        ...
        seen = true;
        result = merge(result, classify_single(part));
...
fn classify_single(cmd: &str) -> Safety {
    ...
    match name {
        "git" => return git_subcommand_safety(cmd),
        "npm" => return npm_subcommand_safety(cmd),
        "cargo" => return cargo_subcommand_safety(cmd),
        _ => {}
    }
...
fn interpreter_safety(cmd: &str) -> Safety {
    ...
    for t in toks {
        ...
        if matches!(lower.as_str(), "-c" | "-i" | "-")
            || (lower == "-e" && !is_python_like)
        {
            saw_inline = true;
            continue;
        }
...
fn is_dangerous_command(cmd: &str) -> bool {
    ...
    if BLOCKED.iter().any(|b| normalized.contains(b)) {
        return true;
    }
...
fn write_mode_present(cmd: &str) -> bool {
    [
        "'w'", "\"w\"", "'a'", "\"a\"", "'x'", "\"x\"", "mode='w'", "mode=\"w\"", "mode='a'",
        "mode=\"a\"", "mode='x'", "'wb'", "'ab'", "'xb'",
    ]
    .iter()
...
mod write_detect_tests {
    ...
    fn blocks_redirect_into_skill() {
        assert!(detect_write_into(
            "cat > /data/skills/foo/scripts/x.py",
            Path::new(ROOT)
        )
        .is_some());
    ...
    fn blocks_cp_mv_into_skill() {
        assert!(detect_write_into("cp /tmp/a.py /data/skills/foo/scripts/a.py", Path::new(ROOT)).is_some());
    ...
    fn allows_read_from_skill() {
        ...
        assert!(detect_write_into("grep bar /data/skills/foo/references/x.md", Path::new(ROOT)).is_none());
        // 从 skill 读、写到 /tmp 也不拦
        assert!(detect_write_into("cp /data/skills/foo/x.py /tmp/y.py", Path::new(ROOT)).is_none());
        assert!(detect_write_into("cat /data/skills/foo/x.py > /tmp/y.py", Path::new(ROOT)).is_none());
    }
    ...
    fn blocks_compound_cp_with_prefix() {
        ...
        assert!(detect_write_into("LD_LIBRARY_PATH=/x && cp a /data/skills/x/b", Path::new(ROOT)).is_some());
        assert!(detect_write_into("export X=y; mv a /data/skills/x/b", Path::new(ROOT)).is_some());
        assert!(detect_write_into("cd /tmp && mkdir -p /data/skills/x/d", Path::new(ROOT)).is_some());
    }
    ...
    fn python_read_of_skill_not_blocked() {
        ...
        assert!(detect_write_into("python3 -c \"open('/data/skills/x/d').read()\"", Path::new(ROOT)).is_none());
        assert!(detect_write_into("python3 -c \"open('/data/skills/x/d','w')\"", Path::new(ROOT)).is_some());
        assert!(detect_write_into("python3 -c \"open('/data/skills/x/d').write_text('a')\"", Path::new(ROOT)).is_some());
    }
}
...
mod tests {
    ...
    fn interpreter_script_own_args_not_inline() {
        ...
        assert_eq!(classify("python manage.py runserver -e prod"), Safety::Allowed);
    ...
    fn interpreter_long_option_eval_needs_prompt() {
        ...
        assert_eq!(classify("node --eval 'require(\"child_process\")'"), Safety::NeedsPrompt);
