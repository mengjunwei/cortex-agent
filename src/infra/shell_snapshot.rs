...
//! ## 与沙箱环境变量的协调
//!
//! 沙箱在 `shell_sandbox` 里把 `TMPDIR`/`XDG_*` 重定向到 workspace 可写子目录。快照里若带回
//! 这些变量的宿主值（只读路径），source 后会覆盖重定向。故捕获时即剔除（`EXCLUDED_VARS`）：
//! source 快照只补充 PATH/venv 等，受管变量仍由进程环境注入、不被覆盖。
...
/// 受沙箱管理的环境变量：source 快照时不能让它们覆盖沙箱重定向（TMPDIR/XDG_* → workspace
/// 可写子目录）；PWD/OLDPWD 影响工作目录；SHLVL/_/PS* 是交互 shell 噪声。捕获时即剔除。
...
pub async fn build(data_dir: &Path, session_id: &str) -> Option<PathBuf> {
    ...
    let raw = match tokio::time::timeout(SNAPSHOT_TIMEOUT, capture_env()).await {
        ...
        Err(_) => {
            tracing::warn!("[shell_snapshot] 捕获 shell 环境超时（>{:?}），跳过快照", SNAPSHOT_TIMEOUT);
            return None;
        }
    };
...
mod tests {
    ...
    fn reconstruct_handles_env0_output() {
        ...
        assert!(out.contains("export HOME='/home/u'"));
        // 受管/噪声变量被剔除
    ...
    fn reconstruct_skips_invalid_names_and_prompt_noise() {
        let raw = "bash-5.1$ \01BAD=x\0GOOD=1\0";
        let out = reconstruct_exports(raw);
    ...
    fn source_prefix_quotes_path() {
        ...
        assert_eq!(source_prefix(Some(p)), ". '/data/snap/abc.sh' 2>/dev/null; ");
    ...
    async fn build_end_to_end_with_real_shell() {
        ...
        let data_dir = std::env::temp_dir().join(format!("cortex-snap-test-{}-{}", std::process::id(), n));
