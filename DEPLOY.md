...
会话运行（/api/run_sse）：读 session 绑定的 assistant_id → 加载助手 → 分发构建 Agent
  ├─ 内置/自定义助手统一走 build_custom_agent（内置设备命令助手由 DB seed 驱动；
  │   monitor_plugin 监控插件助手已暂下线；自定义助手自由组合系统提示词 + 工具 + 生成参数）
  ...
  │            代码工具（read_file / glob / grep / edit_file / create_file，沙箱内）、
  ...
  ├─ 输入富化：消息可带 @ 引用上下文（文件/符号/选区，注入 XML 上下文块）、
  │            图片/文档附件（文档经 markitdown 解析为 Markdown 注入）、$skill-name 提及
...
- **markitdown-mcp**（可选）：会话文档附件解析依赖（pdf/word/excel/ppt/rtf 等转 Markdown 注入对话）。独立部署后以 `slug=markitdown` 注册为外部 MCP（Streamable HTTP），cortex-agent 后端单向可达即可，安装步骤见 [MarkItDown MCP 安装指南](./docs/markitdown-mcp-install.md)
- **对象存储（S3 兼容）**：RustFS / MinIO / AWS S3 任一。**默认启用**，承载截图 / 上传图 / 沙箱快照的共享存储（artifact 暂本地存储，对象存储三期接入——`object_store.rs` 已预留 artifacts key 规范未启用）；多节点部署必须配齐。本地验证可用 `docker run -p 9000:9000 <rustfs/minio 镜像>`，或按 [RustFS 部署指南](./docs/rustfs-deploy.md) 做单机二进制部署（systemd 托管）；仅单机不需要共享文件时可设 `[object_storage].enabled = false` 关闭（截图/上传图/沙箱快照随之不可用）。
...
path = "./logs"
level = "INFO"          # TRACE / DEBUG / INFO / WARN / ERROR（环境变量 RUST_LOG 优先于此，其余值落 info）
otlp_enabled = true     # OTLP 遥测导出（默认 true；部署机无 OTLP 后端时设 false，避免无谓导出失败）
otlp_endpoint = "http://127.0.0.1:5081"  # OTLP gRPC 端点（默认本机 OpenObserve）
...
# 加密密钥已内置源码（src/security/mod.rs 的 APP_SECRETS），不再从配置文件读取。
# AES-256-GCM 静态加密（模型 API Key / MCP 凭据 / KB config / 助手 env_vars / OAuth client_secret）
# 与 JWT HS256 签名共用同一密钥列表，支持轮换（末尾追加新密钥后重启，boot 自动 re-encrypt）。
...
max_inject_chars = 8000         # 单个 skill 正文注入的最大字符数（默认 8000，对齐 codex MAX_SKILL_PROMPT_BYTES，超出截断）
...
sandbox_mode = "workspace-write"  # 文件系统沙箱模式（read-only / workspace-write / danger-full-access，默认 workspace-write）
approval_policy = "unless-trusted" # 命令审批策略（默认 unless-trusted；会话级可覆盖）
network_access = false          # 是否允许命令访问网络（默认 false 禁网；生产需联网装包时显式开启）

[object_storage]
# 对象存储（S3 兼容，接 RustFS/MinIO/AWS S3）——截图/上传图/沙箱快照共享存储
# （artifact 暂本地存储，对象存储三期接入）
...
# 压缩阈值固化为常量（cortex_agent）：软闸 ×0.95 / 硬闸 ×0.95（对齐，达阈值即无条件压缩）/ 上下文剩余提醒 ×0.15
tool_max_output_bytes = 20480        # 工具输出截断阈值（字节，超出语义过滤/硬截断；示例值，代码默认 48KB=49152）
# 其余字段未配时走代码默认：chars_per_token=4 / fallback_context_window=128000 /
# compact_model_id=None（压缩专用便宜模型，None=用主模型）/ max_spawn_depth=3 /
# max_concurrent_children=3 / multi_agent_mode="explicit"（多智能体委派模式：
# explicit=仅用户明确要求才 spawn / proactive=主动委派 / auto=按思考级别推导）

# [agents] 多智能体子代理配置（可选，对齐 codex；缺省时用默认角色表 + 继承父模型）
# [agents.roles.researcher]          # 自定义角色：description / instruction / nickname_candidates
# description = "资料检索与整理"
# [agents]
# default_subagent_model = "<模型id>"  # spawn 未指定 model 时的默认子代理模型（缺省=继承父模型）

[kb]
# 内置知识库 provider（Qdrant）配置；使用 Dify 外挂 provider 时不依赖此段
# 未配置时走代码默认：default_chunk_size=1024 / default_top_k=6 / default_similarity_threshold=0.35
...
>
> `[server]`、`[log]`、`[db]`、`[redis]` 为必填段；`[auth]`、`[skill]`、`[workspace]`、`[shell]`、`[context]`、`[agents]`、`[kb]`、`[mcp]`、`[object_storage]` 缺省时使用各自默认值。
>
...
| `CORTEX_DISABLE_OPENAI_RESPONSES` | 设为 `1` 时关闭 OpenAI 兼容端点的 auto `/responses` 探测协商（构造期读取，纯走 compat `/chat/completions` 路径） |
| `RUST_LOG` | 日志过滤指令（如 `info` / `cortex_agent=debug`），优先于配置文件 `[log].level` |
| `OTEL_EXPORTER_OTLP_HEADERS` | OTLP 遥测上报所需的 Authorization / organization / stream-name |

> 配置加载只读 TOML 文件（`AppConfig::load`），上述以外的配置项均以配置文件为准，无其他业务配置类环境变量注入。

### 3.3 数据库准备

#### 业务表（执行 schema.sql）

所有业务表建表 DDL 统一在 `migrations/schema.sql`（18 张表，含 `users` / `user_identities` / `api_tokens` / `assistants` / `mcp_servers` / `llm_providers` / `llm_models` / **`session_settings`**（会话级配置合并大表：标题/agent_type/模型绑定/思考级别/沙箱+审批/助手绑定 + `user_id` 会话归属隔离 + `token_total`/`token_threshold` 会话级 token 用量快照，取代旧的 `session_models`/`session_assistants`/`session_thinking_levels`/`session_permission_policies` 4 张小表，已 DROP） / `shell_rules` / `kb_instances` / `kb_documents` / `kb_chunks` / `memories` / `memory_proposals` / `monitor_plugins` / `monitor_plugin_versions` / `audit_logs` 等）。cortex-agent 启动时**不再自动建表**，首次部署**必须先执行**：

```bash
psql -d <your_database> -f migrations/schema.sql
```

> 建表段（`CREATE TABLE`）**新建库执行一次即可**——重复执行会因表已存在而报错（非破坏性，不会损坏已有数据）。脚本末尾另有「增量演进」段（`ALTER TABLE ... ADD COLUMN IF NOT EXISTS`），对老库升级幂等可重复执行（自动补列，如 `session_settings.token_total / token_threshold`），新建库执行则为 no-op。
...
# 启动（本仓库是 cargo workspace，共 4 个 bin，无 default-run，必须显式指定 --bin）
cargo run --bin cortex-agent -- --config config/config_1.toml
```

> 4 个 bin：`cortex-agent`（主程序）+ `rhai-runner`（Rhai 脚本子进程执行器，**保留组件，当前生产路径未调用**——监控插件 L2 校验已改为进程内执行）+ `reset_llm_tables`（清空模型表工具）+ `cortex-mcp`（workspace 成员 `crates/cortex-mcp`，内置 stdio MCP 工具集，见 `docs/cortex-mcp.md`）。裸 `cargo run`（不带 `--bin`）会因 package 歧义报错。

> Linux 上 `shell_command` 的 OS 级沙箱依赖 **`bwrap`**（bubblewrap）：文件系统/网络/PID namespace 隔离。`apt install bubblewrap`（Debian/Ubuntu）或 `dnf install bubblewrap`（Fedora/RHEL）。内核需允许非特权 user namespace（`kernel.unprivileged_userns_clone=1`）。建议 ≥0.5（支持 `--as-pid-1` 孤儿进程回收，旧版自动降级）。缺失时禁网/只读/workspace-write 模式的命令 fail-closed 拒绝执行（错误信息会提示安装）。
> 禁网时的 seccomp syscall 级兜底由**主二进制自嵌**完成（`cortex-agent --sandbox-exec-inner`，对齐 codex 单文件模式）——无额外部署文件。
> **读可见性（v1.17.1）**：沙箱对**整盘只读**（`--ro-bind / /`，对齐 codex 默认姿态）——后端用户 shell 里激活的 venv（`$VIRTUAL_ENV`，任意路径如 `/root/python3.13venv`）在沙箱内直接可解析可运行，无需配置。唯一可写区 = 会话工作区；`/tmp` 映射到会话专属宿主目录（跨命令持久、随会话清理）；凭证目录（`~/.ssh ~/.gnupg ~/.aws ~/.azure ~/.kube ~/.docker` + `/etc/ssh`）被 mask 隐藏。文件级凭证（`/etc/shadow`、服务 `config.toml`）仍可读——root 部署下如需收紧，须另行部署文件权限或容器层隔离。

### 4.2 生产模式

```bash
# 1. 先构建前端（rust-embed 在编译期嵌入 static/，release 编译前必须完成，否则嵌入旧产物/空产物）
cd frontend && npm install && npm run build && cd ..
# 产物写入 ../static/（vite outDir），随后编译后端时嵌入

# 2. 一次性编译所有 bin（cortex-agent + rhai-runner + reset_llm_tables + cortex-mcp）
cargo build --release
...
```text
/opt/cortex-agent/
├── cortex_agent           # 主程序（内嵌前端静态资源与沙箱 seccomp helper，单文件）
├── cortex-mcp             # 内置 MCP 工具集（stdio 子进程，需同机部署且 PATH 可达；不启用内置工具可不带）
...
> `rhai-runner` 为保留组件（Rhai 子进程执行器，`SandboxVerifier` 用），当前生产路径无调用方（监控插件 L2 已进程内执行），部署可不带；其查找顺序见 `src/infra/sandbox.rs`（exe 同目录 → target 上溯）。
...
ExecStart=/opt/cortex-agent/cortex_agent --config config/config_1.toml
Restart=always
RestartSec=5
...
> 部署目录参考 4.2 节结构（`ExecStart` 路径须与实际平铺位置一致）。启用内置 cortex-mcp 工具（邮件 / 数据库 / InfluxDB / Prometheus）时记得把 `cortex-mcp` 放在 PATH 可达处。命令沙箱的 seccomp 兜底已内嵌主二进制，无额外部署要求。

### 4.4 多节点部署（会话亲和）

对象存储（截图/上传图/沙箱快照）已解耦到 S3，多节点可共享（artifact 暂本地存储，对象存储三期接入）；但**沙箱工作目录必须留本地 SSD**（`bwrap` 依赖 namespace + bind mount，**不能上 NFS / 对象存储**），故多节点需配置**会话亲和**——保证同一会话始终路由到同一节点；节点故障时新节点从对象存储拉取最新 `workspaces/{sid}/snapshot.tar.zst` 快照恢复续跑（见 [architecture.md §2.4](./docs/architecture.md) `workspace_snapshot`）。
...
| `/api/uploads` | POST | 上传图片 / 文档附件（`multipart/form-data`，字段名 `file`；单文件 ≤ 20MB；图片 png/jpeg/webp/gif + 文档 pdf/doc/docx/xls/xlsx/ppt/pptx/csv/txt/md/rtf，按文件名后缀识别；上传至对象存储 `uploads/{user_id}/...`，返回 presigned URL 直链；认证启用时强制登录） |
| `/api/skills/install` | POST | 安装 Skill（JSON body `{path, overwrite}`，`path` 为工作区内源目录绝对路径且须含 `SKILL.md`；安装后热重载生效） |
| `/api/skills/upload` | POST | 上传 Skill 压缩包安装（`multipart/form-data`，字段 `file` + 可选 `overwrite`；自动识别 zip / tar / tar.gz / .tgz / .tar.xz，单文件 ≤ 50MB） |
| `/api/sessions/{session_id}/files/{path}` | GET | 会话工作区文件下载（shell 工具输出的 `[[ARTIFACT:...]]` 标记文件，前端文件卡片直链；登录态校验会话归属） |
| `/api/kb-instances/{instance_id}/upload-file` | POST | 知识库实例文档上传（`multipart/form-data`，字段 `file` + 可选 `title`/`brand`/`dev_type`/`model`；单文件 ≤ 20MB，支持 pdf/doc/docx/xls/xlsx/ppt/pptx/csv/txt/md/rtf；按实例 provider 分流——Dify 原文件直传、Builtin 先经 markitdown 解析为文本再写入） |
| `/api/kb/proxy-image` | GET | Dify 知识库文档图片代理（`?i=<实例id>&u=<图片URL>`；按实例 `SECRET_KEY` 做 HMAC-SHA256 签名后服务端拉取回传，仅限可信 Dify 文件域） |
| `/api/health` | GET | 健康检查 |
| `/api/v1/monitor/health` | GET | 监控健康检查 |
| `/api/auth/*` | GET/POST | 认证（SSO 跳转 / 回调 / 本地登录 / 注销 + API Token 管理等） |
| `/api/skills/install` · `/api/skills/upload` · `/api/skills/delete` | POST | Skill 安装（工作区绝对路径）/ 压缩包上传（zip/tar，≤50MB）/ 删除 user 级 Skill（多用户模式仅管理员） |
| `/assets/*` | GET | 前端静态资源 |
...
| 流式控制 | `cancelRun`（取消任务）/ `steerRun`（运行中追加输入，无活跃 run 回退正常发送） |
| Skill | `skills`（只读枚举已加载 Skill 目录）/ `reloadSkills`（热重载磁盘目录，新增 Skill 后点一次即可对新会话生效，无需重启） |

> 历史 REST 路由（`/api/sessions`、`/api/kb/*`、`/api/device/search`、`/api/monitor/*`、`/api/catalog`、`/api/agents`、`/api/models`、`/api/cancel`、`/api/brainstorm/*` 等）已全部移除。`web_search` 联网搜索工具、浏览器自动化（zendriver）、头脑风暴、代码助手均已下线/移除。**Skill** 改为文件系统驱动：旧的 DB 持久化管理面（`createSkill` / `updateSkill` 等创建编辑能力）已下线，保留只读枚举 `skills`、热重载 `reloadSkills` 与 REST 删除 `/api/skills/delete`（见上表）。
>
...
读取该助手配置后分发构建 Agent。**所有助手（内置 / 自定义）统一走 `build_custom_agent` 通用路径**，内置助手的 system_prompt / enabled_tools 在 seed 时写入 DB 驱动。内置助手与能力如下：

| 内置助手 | 构建路径 | 能力 | 示例 |
|---------|---------|------|------|
| 设备命令助手 | `build_custom_agent`（DB seed 驱动） | 知识检索 + 结构化命令生成 | "H3C静态路由配置" |
| ~~监控插件助手~~（已下线） | —（`build_monitor_plugin_agent` 代码保留未启用） | 生成 Rhai 监控插件脚本（三层自动校验） | "帮我生成CPU利用率监控插件" |
...
> 内置助手只读、**不支持复制**（`duplicate_builtin` 拒绝内置助手）；自定义助手（`Custom=9`）与内置助手同走通用构建路径 `build_custom_agent`（用户自由组合系统提示词 + shell_command 工具 + 生成参数 + 知识库实例）。
