...
const routes = [
  ...
  { path: '/scheduled-tasks', name: 'ScheduledTasks', component: () => import('../views/ScheduledTaskPage.vue'), meta: { title: '定时任务' } },
  { path: '/scheduled-tasks/:id', name: 'ScheduledTaskDetail', component: () => import('../views/ScheduledTaskDetailPage.vue'), meta: { title: '任务运行记录' } },
  ...
  // 404 兜底：未知路径（含被篡改的 redirect 落点）回首页，而不是渲染空白主布局
 ...
 * 全局前置守卫（强制鉴权）
 *
 * - 首次导航时调用 /api/auth/me 检测登录状态
 * - 已登录态超过 AUTH_REVALIDATE_MS 未核实时重查一次：JWT 过期后后端不回 401
 *   而是回退匿名伪用户（HTTP 仍 200），不主动复核的话长开标签页跨过 TTL 后
 *   所有请求会静默切到 "user" 命名空间且永不跳登录页（重校验把窗口收敛到分钟级）
...
router.beforeEach(async (to) => {
  ...
  if (!userStore.checked) {
    ...
    Date.now() - (userStore.checkedAt || 0) > AUTH_REVALIDATE_MS
  ) {
    await userStore.loadMe()
  }
