            ...
            <div class="model-nested">
              ...
              >
                ...
                <el-table-column label="上下文" width="100" align="center">
                  <template #default="{ row: m }">
                    <span v-if="m.context_window" class="cell-muted">{{ formatContextWindow(m.context_window) }}</span>
...
function openProviderDialog(row) {
  ...
  // 清掉上次打开残留的校验红字（换 model 不重置 el-form 验证态）
...
function openResetKeyDialog(row) {
  ...
  // 清掉上次打开残留的校验红字
  nextTick(() => resetKeyFormRef.value?.clearValidate())
...
function openModelDialog(providerId, row) {
  ...
  // 清掉上次打开残留的校验红字
  nextTick(() => modelFormRef.value?.clearValidate())
...
async function runProbe(ids) {
  ...
  // 兜底文案：请求整体失败 / 回包未覆盖全部请求 id 时，把仍处于 probing 的项
  // 置为 fail，否则徽标永久转圈（旧逻辑失败路径不回滚 probing 状态）
  const failLeftover = (errorText) => {
    const m3 = new Map(probeStatusMap.value)
    for (const id of ids) {
      const cur = m3.get(id)
      if (cur && cur.status === 'probing') {
        m3.set(id, { status: 'fail', error: errorText })
      }
    }
    probeStatusMap.value = m3
  }
  try {
    ...
    if (code === 0) {
      ...
      // 后端可能只返回部分请求 id 的结果（如模型已删/配置缺失）：未覆盖的项不能停在 probing
      failLeftover('No probe result returned')
      probeDrawerVisible.value = true
      // 统计口径以徽标为准（含后端未覆盖、由兜底置 fail 的项），避免 toast 与面板徽标矛盾
      let failCount = 0
      for (const id of ids) {
        if (probeStatusMap.value.get(id)?.status === 'fail') failCount++
      }
      if (failCount === 0) ElMessage.success(`探测完成：${ids.length} 个全部存活`)
      else ElMessage.warning(`探测完成：${failCount} 个失败，详见结果面板`)
    } else {
      failLeftover(message || '探测失败')
      ElMessage.error(message || '探测失败')
    }
  } catch (e) {
    failLeftover(e.message || '网络错误')
    ElMessage.error('请求失败: ' + (e.message || '网络错误'))
  } finally {
