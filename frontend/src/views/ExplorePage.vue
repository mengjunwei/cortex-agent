...
// Fork 需要登录身份（匿名 Fork 的副本会挂到后端伪用户 "user" 名下，登录后消失）。
// 「后端未启用 auth」时放行——镜像路由守卫语义，单机免鉴权部署不受影响
const authOptional = computed(
  () => !userStore.localEnabled && userStore.providers.length === 0,
)
function requireLoginToFork() {
  if (userStore.authenticated || authOptional.value) return true
