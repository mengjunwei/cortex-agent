        ...
        >
          <div class="version-top">
            ...
            <div class="version-actions">
              <!-- 源码接口只返回当前生效版本，历史版本行不提供按钮以免误导
                   （显示的是当前版源码而非该行版本的源码） -->
              <el-button
                v-if="v.version === activeVersion"
                size="small"
                :loading="sourceLoading"
                @click="handleViewSource(pluginId)"
              >查看源码</el-button>
              <el-button
                v-if="v.version !== activeVersion"
                size="small"
                type="warning"
                plain
                :loading="rollbackVersion === v.version"
...
// 正在回滚的版本号（按行显示 loading；非空时其余回滚按钮禁用，防并发回滚）
...
// 请求序号：路由参数变化触发重载时，旧 plugin 的回包可能后到，覆盖新 plugin 的版本列表
let loadVersionsSeq = 0

async function loadVersions() {
  if (!pluginId.value) return
  const seq = ++loadVersionsSeq
  versionsLoading.value = true
  try {
    const { data, code, message } = await fetchPluginVersions(pluginId.value)
    if (seq !== loadVersionsSeq) return
    if (code === 0) {
    ...
    if (seq === loadVersionsSeq) ElMessage.error('请求失败: ' + (e.message || '网络错误'))
  } finally {
    if (seq === loadVersionsSeq) versionsLoading.value = false
  }
}
