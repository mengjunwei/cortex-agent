    ...
    <el-dialog v-model="dialogVisible" :title="editing ? '编辑记忆' : '添加记忆'" width="520px">
      <el-form label-width="80px">
        ...
        <el-form-item v-if="form.scope === 1" label="助手">
          <el-select v-model="form.assistant_id" placeholder="选择该记忆绑定的助手" filterable style="width: 100%">
...
async function save() {
  ...
  if (f.scope === 1 && !f.assistant_id) {
    ElMessage.warning('助手级记忆需要选择助手')
    return
  }
  saving.value = true
  const payload = { type: f.type, scope: f.scope, content: f.content.trim() }
  if (f.scope === 1) payload.assistant_id = f.assistant_id
