...
// ── URL query 与本地状态双向同步 ──
// setup 阶段（watch 注册前）从 URL 恢复：刷新 / 从详情页返回都能保持筛选状态；
// 放在 onMounted 恢复会晚于下方「筛选变化重置页码」的 watch 注册，恢复赋值会被
// 误判为用户筛选而把 URL 里的页码重置掉
restoreFromQuery()
...
// 筛选条件变化即重置页码：列表是 computed 实时过滤，输入关键字时结果可能只剩
// 一页，停在旧页码会看到空表格（旧逻辑只在回车/清空时重置）
...
// 页码越界收敛：注销插件后重载 / 筛选结果变少时，currentPage 可能超出最大页，
// 停在越界页会渲染空表格（含 URL 恢复出已失效的旧页码场景）
watch(allPlugins, (list) => {
