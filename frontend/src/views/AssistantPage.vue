  ...
  <div class="page">
    ...
    <section class="block">
      <div class="block-head">
        <h2 class="block-title">内置助手</h2>
        <span class="block-count">{{ builtinAssistants.length }} 个</span>
      </div>
      <div v-loading="assistantStore.loading" class="card-grid">
        <article
          v-for="a in builtinAssistants"
          :key="a.id"
          class="asst-card builtin"
        >
          <div class="ac-head">
            <div class="ac-avatar">{{ a.avatar || '🤖' }}</div>
            <div class="ac-title-wrap">
              <h3 class="ac-title">{{ a.name }}</h3>
              <el-tag size="small" effect="plain" round>{{ agentTypeLabel(a) }}</el-tag>
            </div>
          </div>
        ...
        <el-empty v-if="!builtinAssistants.length && !assistantStore.loading" description="暂无内置助手（数据库未启用）" />
      </div>
    </section>
    ...
    <section class="block">
      <div class="block-head">
        ...
        <span class="block-count">{{ customAssistants.length }} 个</span>
      </div>
      <div class="card-grid">
        <article
          v-for="a in customAssistants"
          :key="a.id"
          class="asst-card custom"
        >
          <div class="ac-head">
            <div class="ac-avatar">{{ a.avatar || '🤖' }}</div>
            <div class="ac-title-wrap">
              <h3 class="ac-title">{{ a.name }}</h3>
              <div class="ac-tags">
                <el-tag size="small" type="success" effect="plain" round>自定义</el-tag>
                <el-tag
                  v-if="a.visibility === 1"
                  size="small"
                  type="warning"
        ...
        <el-empty
          v-if="!customAssistants.length && !assistantStore.loading"
          description="还没有自定义助手，点击右上角「创建助手」开始"
        >
...
function agentTypeLabel(a) {
  return AGENT_TYPE_KEY_LABEL[a.agent_type_key] || '自定义'
}
