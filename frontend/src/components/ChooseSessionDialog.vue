...
// 请求序号：防抖期间快速连续改关键词会叠加多个请求，先发的后到会把新结果覆盖成旧结果
let querySeq = 0
...
async function queryFirst() {
  ...
  try {
    ...
    if (seq !== querySeq) return // 已被更新的搜索取代，丢弃过期回包
    if (code === 0) {
...
async function queryMore() {
  ...
  // 与 queryFirst 共用 querySeq：加载更多在途时用户清关键词/改搜索触发了 queryFirst，
  // 过期的后续页会 push 进新结果集（页码错乱且污染持续），序号对齐后丢弃过期回包
  const seq = querySeq
  ...
  try {
    ...
    if (seq !== querySeq) return // 已被新的首屏查询取代，丢弃
    if (code === 0) {
