<template>
  <div class="tool-card" :class="cardClass">
    ...
    <template v-if="isShellCall(msg)">
      ...
      <!-- 审批内嵌条（chatStore.pendingShellApproval 挂载到本卡时显示） -->
    ...
    <div v-else class="tool-body">
      <template v-if="!canToggle || msg._expanded">
        ...
        <!-- grep 摘要 -->
        <div v-else-if="msg._grepSummary" class="tool-section">
          <el-alert
            type="info"
            :closable="false"
            show-icon
            :title="msg._grepSummary.summary.note"
            class="grep-alert"
          />
          <div class="grep-summary-list">
            <div v-for="(f, fi) in msg._grepSummary.files" :key="fi" class="grep-summary-item">
              <code class="grep-file">{{ f.file }}</code>
              <el-tag size="small" type="info">{{ f.count }} 处</el-tag>
            </div>
          </div>
        </div>

        <!-- list_directory 折叠目录 -->
        <div v-else-if="msg._collapsedEntries && msg._collapsedEntries.length" class="tool-section">
          <div class="collapsed-hint-list">
            <div v-for="(c, ci) in msg._collapsedEntries" :key="ci" class="collapsed-hint-item">
              <span class="collapsed-name"><Folder :size="13" /> {{ c.name }}/</span>
              <el-tag size="small" type="info" effect="plain">{{ c.collapsed_count }} 项已折叠</el-tag>
            </div>
          </div>
        </div>

        <!-- 通用 JSON 结果 -->
        <div v-else-if="msg.result" class="tool-section">
...
import { parseAny, stripArtifact } from '../../utils/toolResult'
...
const iconComp = computed(() => {
  ...
  if (n === 'list_directory' || n === 'List' || n.includes('列出目录') || n.includes('目录')) return Folder
...
// 是否可折叠：仅「无特化渲染」的兜底工具卡可折叠；特化卡（终端/校验/注册/截图…）默认展开
const canToggle = computed(() => {
  ...
  if (m._grepSummary) return false
  if (m._collapsedEntries && m._collapsedEntries.length) return false
  return true
})
...
function getValidateResult(result) {
  ...
  if (!obj || !Array.isArray(obj.cases) || !('passed_cases' in obj)) return null
  return {
    ok: obj.ok === true,
...
function getRegisterResult(result) {
  const obj = parseAny(result)
  if (!obj || !('plugin_id' in obj)) return null
  return {
    ok: obj.ok === true,
...
function decideApproval(approved) {
  ...
  chatStore.resolveShellApproval(approved)
}
