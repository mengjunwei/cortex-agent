<template>
  <!-- codex 风格探索块：连续的 read_file / glob / grep 聚合，
...
function argText(m) {
  ...
  if (k === 'glob') {
    ...
    const p = a.path && a.path !== '.' ? ` in ${a.path}` : ''
    return `${q}${p}`
  }
  if (k === 'search') {
    const q = a.pattern ? `"${a.pattern}"` : ''
...
function outputOf(m) {
  ...
  // glob: matches 数组（元素带 path 字段；grep 的 matches 元素带 file 字段）
  if (Array.isArray(r.matches) && r.matches.some((mt) => mt && typeof mt.path === 'string')) {
    return r.matches.map((mt) => `${mt.kind === 'dir' ? '📁' : '📄'} ${mt.path}`).join('\n')
  }
  // grep: matches 数组（后端 Match 结构：{ file, line_no: 行号, line: 行文本 }）
  if (Array.isArray(r.matches)) {
    return r.matches
      .map((mt) => `${mt.file || ''}${mt.line_no != null ? `:${mt.line_no}` : ''}: ${mt.line || ''}`)
      .join('\n')
  }
  // grep output_mode=files_with_matches: 文件列表
  if (Array.isArray(r.files)) return r.files.join('\n')
  // grep output_mode=count: 每文件命中数
  if (Array.isArray(r.counts)) {
    return r.counts.map((c) => `${c.file || ''}: ${c.count ?? ''}`).join('\n')
  }
