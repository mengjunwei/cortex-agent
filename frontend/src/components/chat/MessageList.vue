  ...
  <template v-for="(group, idx) in grouped" :key="idx">
    <div class="message-row msg-enter" :style="{ animationDelay: Math.min(idx * 35, 350) + 'ms' }" :id="'msg-' + group.origIdx" :class="group.kind === 'single' ? group.msg.role : 'tool_call'">
      ...
      <div v-else-if="group.msg.role === 'compacted'" class="compact-divider">
        ...
        <span class="compact-divider-text">💡 对话较长，已自动精简早期内容</span>
        <span class="compact-divider-line"></span>
      </div>
      <div v-else-if="group.msg.role === 'model_switched'" class="compact-divider">
        <span class="compact-divider-line"></span>
        <span class="compact-divider-text">🔄 模型已切换 {{ group.msg.from }} → {{ group.msg.to }}</span>
        <span class="compact-divider-line"></span>
      </div>
    </div>
  </template>
...
// 探索类工具（对齐 codex exploring cell）：read_file / glob / grep，连续聚合成「Exploring」块。
// 工具名兼容 codex 短词(Read/Glob/Search，由后端 tool_display 下发——实时流与历史恢复
// 两条路径都发短词，原始英文名实际到不了前端，仅为语义完整保留)与旧历史的中文名
// (读取文件/搜索内容)（list_directory 已下线，不留兼容名）。
