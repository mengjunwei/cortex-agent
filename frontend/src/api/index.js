...
// 上传图片附件（multipart），返回 { url, filename, mime_type, size }
...
// 上传文档附件（Word/Excel/PPT/PDF 等，后端走 markitdown 解析），返回 { url, filename, mime_type, size }
...
// REST 通用辅助（统一信封 { code, message, data }），用于不走 GraphQL 的接口（如定时任务）
async function rest(method, url, body) {
  ...
  return {
    ...
    code: env && typeof env.code === 'number' ? env.code : (resp.ok ? 0 : -1),
    message: env && typeof env.message === 'string' ? env.message : '',
  }
}

// ── 定时任务（REST）──────────────────────────
export const parseSchedule = (scheduleNl, timezone) =>
  rest('POST', '/api/scheduled-tasks/parse-schedule', { schedule_nl: scheduleNl, timezone })

export const fetchScheduledTasks = (page = 1, pageSize = 10) =>
  rest('GET', `/api/scheduled-tasks?page=${page}&page_size=${pageSize}`)

export const fetchScheduledTask = (id) =>
  rest('GET', `/api/scheduled-tasks/${id}`)

export const createScheduledTask = (payload) =>
  rest('POST', '/api/scheduled-tasks', payload)

export const updateScheduledTask = (id, payload) =>
  rest('PATCH', `/api/scheduled-tasks/${id}`, payload)

export const deleteScheduledTask = (id) =>
  rest('DELETE', `/api/scheduled-tasks/${id}`)

export const fetchScheduledTaskRuns = (id, page = 1, pageSize = 10) =>
  rest('GET', `/api/scheduled-tasks/${id}/runs?page=${page}&page_size=${pageSize}`)

export const runScheduledTaskNow = (id) =>
  rest('POST', `/api/scheduled-tasks/${id}/run-now`)
...
// 运行中注入新消息（codex 式 steer）：服务端入队当前 run，下轮模型请求前生效；
// 返回 { steered: true/false }，false = run 已结束（竞态），前端应回退为正常发送
export const steerRun = (threadId, messages, runId) =>
  gql(
    `mutation($threadId: String!, $messages: JSON!, $runId: String) { steerRun(threadId: $threadId, messages: $messages, runId: $runId) }`,
    { threadId, messages, runId },
  )
...
export const revealAssistantEnvVars = (id, password) =>
  gql(
    `mutation($id: String!, $password: String!) {
      revealAssistantEnvVars(id: $id, password: $password)
    }`,
    { id, password },
  )
...
// 上传文档文件（multipart，后端走 markitdown 解析为文本再写入知识库）。
// meta: { title?, brand?, dev_type?, model? }，均可选。
// 返回与 GraphQL 统一信封一致：{ code, message, data }。
export const uploadInstanceDocumentFile = async (instanceId, file, meta = {}) => {
  const fd = new FormData()
  fd.append('file', file)
  if (meta.title) fd.append('title', meta.title)
  if (meta.brand) fd.append('brand', meta.brand)
  if (meta.dev_type) fd.append('dev_type', meta.dev_type)
  if (meta.model) fd.append('model', meta.model)
  const resp = await fetch(`/api/kb-instances/${instanceId}/upload-file`, {
    method: 'POST',
    body: fd,
  })
  return resp.json()
...
/** 登录态修改密码（校验原密码；成功后该账号全部会话失效，需重新登录） */
...
/** 删除 user 级 Skill(REST,仅管理员;后端删目录+热重载) */
